...
const envSchema = z
  .object({
    ...
    CLICKHOUSE_AUDIT_LOG_INSERT_SETTINGS: zpStr(
      z
        ...
        .default('{"async_insert":1,"wait_for_async_insert":1,"date_time_input_format":"best_effort"}')
        .describe(
    ...
    KMS_PRIMARY_INSTANCE_URL: zpStr(z.string().optional()),
  ...
  .transform((data) => ({
    ...
    isSecondaryInstance: Boolean(data.KMS_PRIMARY_INSTANCE_URL),
    isHsmConfigured:
