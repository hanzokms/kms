...
<Tabs>
  <Tab title="Single command">
    ```bash
    hanzo-kms run [options] -- [your application start command]

    # Example
    hanzo-kms run [options] -- npm run dev
    ```
  ...
  <Tab title="Chained commands">
    ```bash
    hanzo-kms run [options] --command [string command]

    # Example
    hanzo-kms run [options] --command "npm run bootstrap && npm run dev start; other-bash-command"
    ```
...
<Accordion title="hanzo-kms run" defaultOpen="true">
  Use this command to inject secrets into your applications process

```bash
$ hanzo-kms run -- <your application command>
...
### Flags
  ...
  <Accordion title="--project-config-dir">
    ...
    ```bash
      # Example
      hanzo-kms run --project-config-dir=/some-dir -- printenv
    ```
  </Accordion>
  ...
  <Accordion title="--command">
    ...
    ```bash
    # Example
    hanzo-kms run --command="npm run build && npm run dev; more-commands..."
    ```
  ...
  <Accordion title="--projectId">
    ...
    ```bash
    # Example
    hanzo-kms run --projectId=<project-id> -- npm run dev
    ```
  ...
  <Accordion title="--token">
    ...
    ```bash
    # Example
    hanzo-kms run --token="<universal-auth-access-token>" --projectId=<project-id> -- npm run start
    ```
...
<Accordion title="--env">
  ...
  <Accordion title="--tags">
    ...
    ```bash
    # Example
    hanzo-kms run --tags=tag1,tag2,tag3 -- npm run dev
    ```
  ...
  <Accordion title="--path">
    ...
    ```bash
    # Example
    hanzo-kms run --path="/nextjs" -- npm run dev
    ```
...
```bash
hanzo-kms run --watch -- npm run dev
```
