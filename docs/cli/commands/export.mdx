...
<Accordion title="hanzo-kms export" defaultOpen="true">
...
hanzo-kms export > .env
hanzo-kms export --output-file=./.env

# Export variables to a .env file (with export keyword)
hanzo-kms export --format=dotenv-export > .env
hanzo-kms export --format=dotenv-export --output-file=./.env

# Export variables to a JSON file
hanzo-kms export --format=json > secrets.json
hanzo-kms export --format=json --output-file=./secrets.json

# Export variables to a YAML file
hanzo-kms export --format=yaml > secrets.yaml
hanzo-kms export --format=yaml --output-file=./secrets.yaml

# Render secrets using a custom template file
hanzo-kms export --template=<path to template>
```
...
### flags
  ...
  <Accordion title="--output-file">
    ...
    hanzo-kms export --format=yaml --output-file=./
    ```
    ...
    **When `--output-file` is NOT specified (default behavior):**
    ...
    - You can use shell redirection like `hanzo-kms export > secrets.json`
  ...
  <Accordion title="--template">
    ...
    ```bash
    # Example
    hanzo-kms export --template="/path/to/template/file"
    ```
  ...
  <Accordion title="--projectId">
    ...
    hanzo-kms export --projectId=XXXXXXXXXXXXXX
    ```
  ...
  <Accordion title="--path">
    ...
    ```bash
    # Example
    hanzo-kms export --path="/path/to/folder" --env=dev
    ```
  ...
  <Accordion title="--tags">
    ...
    ```bash
    # Example
    hanzo-kms export --tags=tag1,tag2,tag3 --env=dev
    ```
