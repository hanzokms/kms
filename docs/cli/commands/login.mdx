...
<ParamField query="Flags">
  ...
  <AccordionGroup>
      ...
      <Accordion title="Direct Login (CI/CD)">
        ```bash
        hanzo-kms login --email=user@example.com --password=your-password --organization-id=your-organization-id
        
        # Or using environment variables
        export KMS_EMAIL="user@example.com"
        export KMS_PASSWORD="your-password"
        export KMS_ORGANIZATION_ID="your-organization-id"
        hanzo-kms login
        ```
      </Accordion>
      <Accordion title="Interactive CLI Login">
        ```bash
        hanzo-kms login --interactive
        ```
      </Accordion>
      <Accordion title="Plain Token Output (Useful for scripting and CI/CD)">
        ```bash
        export KMS_TOKEN=$(hanzo-kms login --email=user@example.com --password=your-password --organization-id=your-organization-id --plain --silent)
        ```
      </Accordion>
    </AccordionGroup>
</Accordion>
...
<Accordion title="Universal Auth">
  ...
  <Steps>
    ...
    ```bash
      hanzo-kms login --method=universal-auth --client-id=<client-id> --client-secret=<client-secret>
    ```
    </Step>
...
<Accordion title="Native Kubernetes">
  ...
  <Steps>
    ...
    ```bash
      ...
      hanzo-kms login --method=kubernetes --machine-identity-id=<machine-identity-id> --service-account-token-path=<service-account-token-path>
    ```
    </Step>
  ...
  <Accordion title="Native Azure">
    ...
    <Steps>
      ...
      ```bash
        hanzo-kms login --method=azure --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="Native GCP ID Token">
    ...
    <Steps>
      ...
      ```bash
        hanzo-kms login --method=gcp-id-token --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="GCP IAM">
    ...
    <Steps>
      ...
      ```bash
        hanzo-kms login --method=gcp-iam --machine-identity-id=<machine-identity-id> --service-account-key-file-path=<service-account-key-file-path>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="Native AWS IAM">
    ...
    <Steps>
      ...
      ```bash
        hanzo-kms login --method=aws-iam --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="OIDC Auth">
    ...
    <Steps>
      ...
      ```bash
        hanzo-kms login --method=oidc-auth --machine-identity-id=<machine-identity-id> --jwt=<oidc-jwt>
      ```
      </Step>
    </Steps>
  ...
  <Accordion title="JWT Auth">
    ...
    <Steps>
      ...
      ```bash
        hanzo-kms login --method=jwt-auth --jwt=<jwt-token> --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
...
<AccordionGroup>
  <Accordion title="--method">
    ```bash
    hanzo-kms login --method=<auth-method> # Optional, will default to 'user'.
    ```
  ...
  <Accordion title="--organization-slug">
    ```bash
    hanzo-kms login --method=<auth-method> --organization-slug="team2" 
    ```
   #### Description
    This authenticates the machine identity against the specified sub-organization.

    <Tip>
      The `organization-slug` flag can be substituted with the `KMS_AUTH_ORGANIZATION_SLUG` environment variable.
    </Tip>
  ...
  <Accordion title="--client-id">
    ```bash
    hanzo-kms login --client-id=<client-id> # Optional, required if --method=universal-auth.
    ```
  ...
  <Accordion title="--client-secret">
    ```bash
    hanzo-kms login --client-secret=<client-secret> # Optional, required if --method=universal-auth.
    ```
    #### Description
  ...
  <Accordion title="--machine-identity-id">
    ```bash
    hanzo-kms login --machine-identity-id=<your-machine-identity-id> # Optional, required if --method=kubernetes, azure, gcp-id-token, gcp-iam, or aws-iam.
    ```
  ...
  <Accordion title="--service-account-token-path">
    ```bash
    hanzo-kms login --service-account-token-path=<service-account-token-path> # Optional Will default to '/var/run/secrets/kubernetes.io/serviceaccount/token'.
    ```
  ...
  <Accordion title="--service-account-key-file-path">
    ```bash
    hanzo-kms login --service-account-key-file-path=<gcp-service-account-key-file-path> # Optional, but required if --method=gcp-iam.
    ```
  ...
  <Accordion title="--email">
    ```bash
    hanzo-kms login --email=<email> --password=<password> --organization-id=<organization-id>
    ```
    ...
    <Tip>
      The `email` flag can be substituted with the `KMS_EMAIL` environment variable.
    </Tip>
  ...
  <Accordion title="--password">
    ...
    <Warning>
      For security in CI/CD environments, prefer using the `KMS_PASSWORD` environment variable instead of passing the password as a command-line flag.
    </Warning>
    ...
    <Tip>
      The `password` flag can be substituted with the `KMS_PASSWORD` environment variable.
    </Tip>
    ...
    <Tip>
      The `organization-id` flag can be substituted with the `KMS_ORGANIZATION_ID` environment variable.
    </Tip>
  ...
  <Accordion title="--plain">
    ```bash
    hanzo-kms login --email=<email> --password=<password> --organization-id=<organization-id> --plain
    ```
    ...
    export KMS_TOKEN=$(hanzo-kms login --email=<email> --password=<password> --organization-id=<organization-id> --plain --silent)
    ```
  ...
  <Accordion title="--jwt">
    ```bash
    hanzo-kms login --jwt=<jwt-token> --machine-identity-id=<machine-identity-id>
    ```
    ...
    <Tip>
    The `jwt` flag can be substituted with the `KMS_JWT` environment variable.
    </Tip>

  </Accordion>
  <Accordion title="--domain">
    ```bash
    hanzo-kms login --domain=<domain-url>
    ```

    #### Description
    Specifies the KMS API URL for non-US Cloud instances. This flag is required when connecting to any instance other than US Cloud (e.g. EU Cloud or self-hosted).

    ```bash
    # Example for EU Cloud
    hanzo-kms login --domain="https://eu.kms.hanzo.ai"

    # Example for localhost
    hanzo-kms login --domain="http://localhost:8080"

    # Example for self-hosted
    hanzo-kms login --domain="https://your-self-hosted-kms.hanzo.ai"
    ```

    <Warning>
      **Critical:** If you use `--domain` during login, you must also include it on **all subsequent CLI commands** (e.g., `hanzo-kms secrets`, `hanzo-kms export`, etc.). Alternatively, set the `KMS_API_URL` environment variable to avoid having to use `--domain` on every command. Refer to the [Domain Configuration](/cli/usage#domain-configuration) section for more details.
    </Warning>
...
<AccordionGroup>
  <Accordion title="Browser Login (Default)">
    By default, running `hanzo-kms login` without any flags opens your browser for authentication.
    ...
    The browser will open to the KMS login page, and upon successful authentication, the CLI will be automatically authenticated.
  ...
  <Accordion title="Direct Login (Non-Interactive)">
    ...
    hanzo-kms login --domain https://eu.kms.hanzo.ai --email user@example.com --password "your-password" --organization-id "your-organization-id"

    # Basic direct login (Self-hosted Instance)
    hanzo-kms login --domain https://your-self-hosted-kms.hanzo.ai --email user@example.com --password "your-password" --organization-id "your-organization-id"
    ...
    export KMS_TOKEN=$(hanzo-kms login --plain --silent)
    ```

    <Warning>
      **For non-US Cloud instances:** If you're using EU Cloud or a self-hosted instance, you must set `KMS_API_URL` before login or use `--domain` on all commands. Refer to the [Domain Configuration](/cli/usage#domain-configuration) section for more details.
    </Warning>
...
<Steps>
  <Step title="Obtain an access token">
        ```bash
          export KMS_TOKEN=$(hanzo-kms login --method=universal-auth --client-id=<client-id> --client-secret=<client-secret> --silent --plain) # silent and plain is important to ensure only the token itself is printed, so we can easily set it as an environment variable.
        ```
      ...
      <Step title="Fetch all secrets from an environment">
        ```bash
          hanzo-kms secrets --projectId=<your-project-id> --env=dev --recursive
        ```
