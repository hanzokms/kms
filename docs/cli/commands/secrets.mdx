...
<Accordion title="hanzo-kms secrets" defaultOpen="true">
...
### Flags
  ...
  <Accordion title="--projectId">
   The project ID to fetch secrets from. This is required when using a machine identity to authenticate.
    ...
    ```bash
    # Example
    hanzo-kms secrets --projectId=<project-id>
    ```
  ...
  <Accordion title="--path">
    ...
    ```bash
    # Example
    hanzo-kms secrets --path="/" --env=dev
    ```
  ...
  <Accordion title="--plain">
    ...
    ```bash
    # Example
    hanzo-kms secrets --plain --silent
    ```
...
<Accordion title="hanzo-kms secrets get">
  This command allows you selectively print the requested secrets by name

```bash
$ hanzo-kms secrets get <secret-name-a> <secret-name-b> ...

# Example
$ hanzo-kms secrets get DOMAIN
$ hanzo-kms secrets get DOMAIN PORT
...
### Flags
  ...
  <Accordion title="--plain">
    ...
    ```bash
    # Example
    hanzo-kms secrets get FOO --plain
    hanzo-kms secrets get FOO BAR --plain

    # Fetch a single value and assign it to a variable
    API_KEY=$(hanzo-kms secrets get FOO --plain --silent)
    ```
  ...
  <Accordion title="--silent">
    ...
    ```bash
    # Example
    hanzo-kms secrets get FOO --plain --silent
    ```
  ...
  <Accordion title="--raw-value (deprecated)">
    ...
    Example: `hanzo-kms secrets get DOMAIN --raw-value`
...
<Accordion title="hanzo-kms secrets set">
...
```bash
$ hanzo-kms secrets set <key1=value1> <key2=value2> <key3=@/path/to/file>...

## Example
$ hanzo-kms secrets set STRIPE_API_KEY=sjdgwkeudyjwe DOMAIN=example.com HASH=jebhfbwe SECRET_PEM_KEY=@secret.pem
```
...
### Flags
  ...
  <Accordion title="--path">
    ...
    ```bash
    # Example
    hanzo-kms secrets set DOMAIN=example.com --path="common/backend"
    ```
  ...
  <Accordion title="--type">
    ...
    ```bash
    # Example
    hanzo-kms secrets set DOMAIN=example.com --type=personal
    ```
  ...
  <Accordion title="--file">
    ...
    ```bash
    # Example
    hanzo-kms secrets set --file="./.env"
    ```
  </Accordion>
</Accordion>

<Accordion title="hanzo-kms secrets delete">
  This command allows you to delete secrets by their name(s).

```bash
$ hanzo-kms secrets delete <keyName1> <keyName2>...

## Example
$ hanzo-kms secrets delete STRIPE_API_KEY DOMAIN HASH
```
...
### Flags
  ...
  <Accordion title="--path">
    ...
    ```bash
    # Example
    hanzo-kms secrets delete <keyName1> <keyName2>... --path="/"
    ```

  </Accordion>
</Accordion>

<Accordion title="hanzo-kms secrets folders">
  This command allows you to fetch, create and delete folders from within a path from a given project.

```bash
$ hanzo-kms secrets folders
```
...
### sub commands
  ...
  <Accordion title="get">
    ...
    ```
    hanzo-kms secrets folders get --path=/some/path/to/folder 
    ```
    #### Flags 
    <Accordion title="--path">
  ...
  <Accordion title="create">
    ...
    ```
    hanzo-kms secrets folders create --path=/some/path/to/folder --name=folder-name
    ```
    ### Flags 
    <Accordion title="--path">
  ...
  <Accordion title="delete">
    ...
    ```
    hanzo-kms secrets folders delete --path=/some/path/to/folder --name=folder-name
    ```
    ### Flags 
    <Accordion title="--path">
      Path to where the folder should be created
...
<Accordion title="hanzo-kms secrets generate-example-env">
...
```bash
$ hanzo-kms secrets generate-example-env

## Example
$ hanzo-kms secrets generate-example-env > .example-env
```
