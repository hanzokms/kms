...
The KMS gateway provides secure access to private resources using modern TCP-based SSH tunnel architecture with enhanced security and flexible deployment options.
...
<AccordionGroup>
<Accordion title="kms gateway start" defaultOpen="true">
  Run the KMS gateway component within your the network where your target resources are located. The gateway establishes an SSH reverse tunnel to a relay server and provides secure access to private resources within your network.
...
- Authenticate using SSH certificates issued by KMS
...
<Accordion title="kms gateway systemd install">
...
<AccordionGroup>
<Accordion title="kms gateway (deprecated)">
  <Warning>
...
Run the legacy KMS gateway in the foreground. The gateway will connect to the relay service and maintain a persistent connection.
...
<Accordion title="kms gateway install (deprecated)">
  <Warning>
  **This command is deprecated and will be removed in a future release.**
