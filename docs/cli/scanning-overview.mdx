...
<Tabs>
  <Tab title="Scanning files, directories and or git history">
	...
	The `hanzo-kms scan` command serves to scan repositories, directories, and files. It's compatible with both individual developer machines and Continuous Integration (CI) environments.

	When you run `hanzo-kms scan` on a Git repository, KMS will parses the output of a `git log -p` command. This command generates [patches](https://stackoverflow.com/questions/8279602/what-is-a-patch-in-git-version-control) that KMS uses to identify secrets in your code.
	...
	For instance, to instruct KMS to scan a specific range of commits, use the following command: `hanzo-kms scan --log-opts="--all commitA..commitB"`. For more details, refer to the [Git log documentation](https://git-scm.com/docs/git-log).
  ...
  <Tab title="Scanning uncommitted files ">
	```bash
	hanzo-kms scan git-changes

	# Display the full secret findings
	hanzo-kms scan git-changes --verbose
	```
...
```bash
hanzo-kms scan git-changes --staged --verbose
```
...
A baseline allows KMS to ignore any old findings that are already present in the baseline findings. You can create a hanzo-kms scan report by running `hanzo-kms scan` with the `--report-path` flag.

To create a KMS scan report and save it in a file called leaks-report.json, use the following command:

```
hanzo-kms scan --report-path leaks-report.json
```

Once a baseline is created, you can apply it when running the `hanzo-kms scan` command again. Use the following command:

```
hanzo-kms scan --baseline-path leaks-report.json --report-path findings.json
```
...
If you're intentionally committing a test secret that `hanzo-kms scan` might flag, you can instruct KMS to overlook that secret with the methods listed below.
