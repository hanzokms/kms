...
description: "Complete guide to deploying KMS Gateways including network configuration and firewall requirements"
---

KMS Gateways enables secure communication between your private resources and the KMS platform without exposing inbound ports in your network.
This guide covers everything you need to deploy and configure KMS Gateways.
...
<Steps>
  <Step title="Provision a Machine Identity">
    Create a machine identity with the correct permissions to create and manage gateways. This identity is used by the gateway to authenticate with KMS and should be provisioned in advance.
  ...
  - **Managed relay (KMS Cloud, US/EU only):** Managed relays are only available for KMS Cloud instances in the US and EU regions. If you are using KMS Cloud in these regions, you can use the provided managed relay.
  - **Self-hosted relay:** For all other cases, including all self-hosted and dedicated enterprise instances of KMS, you must deploy your own relay server. You can also choose to deploy your own relay server when using KMS Cloud if you require reduced geographic proximity to your target resources for lower latency or to reduce network congestion. For setup instructions, see the [Relay Deployment Guide](/documentation/platform/gateways/relay-deployment).
  </Step>
  ...
  <Step title="Configure Network & Firewall">
    ...
    If you are in a corporate environment with strict egress filtering, ensure outbound TCP 2222 to relay servers and outbound HTTPS 443 to KMS API endpoints are allowed.
  </Step>
  <Step title="Select a Deployment Method">
    ...
    <Tabs>
      ...
      <Tab title="Kubernetes (Production)">
        For production deployments on Kubernetes clusters, install the Gateway using the KMS Helm chart:
        ...
        kubectl create secret generic kms-gateway-environment \
          --from-literal=KMS_AUTH_METHOD=universal-auth \
          --from-literal=KMS_UNIVERSAL_AUTH_CLIENT_ID=<client-id> \
          --from-literal=KMS_UNIVERSAL_AUTH_CLIENT_SECRET=<client-secret> \
          --from-literal=KMS_GATEWAY_NAME=<gateway-name>
        ```

        <Info>
          By default, the gateway connects to the most optimal relay. Use the `--from-literal=KMS_RELAY_NAME=<relay-name>` flag to manually specify a different relay server.
        </Info>
      ...
      <Tab title="Development & Testing">
        ...
        ```bash
        sudo kms gateway start --token <token> --name=<gateway-name>
        ```
  ...
  <Step title="Verify Your Gateway Deployment">
    After deployment, verify your gateway is working:
      ...
      2. **Verify registration** in the KMS by visiting the Gateways section of your organization. The new gateway should appear with a recent heartbeat timestamp.

      3. **Test connectivity** by creating a resource in KMS that uses the gateway to access a private service. Verify the resource can successfully connect through the gateway.
  </Step>
</Steps>
...
- **Outbound HTTPS** to KMS API endpoints on port 443
...
1. Test SSH port to relay:
  ...
  nc -zv <relay-ip> 2222
  ```
2. Test outbound API access (replace with your KMS domain if different):
  ```bash
...
- **systemd service:**
  ```bash
  sudo journalctl -u kms-gateway -f
  ```
- **Kubernetes:**
  ```bash
