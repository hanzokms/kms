...
<Info>
    ...
    If you're using KMS Cloud, then it is available under the **Enterprise Tier**. If you're self-hosting KMS, then you should contact team@kms.hanzo.ai to purchase an enterprise license to use it.
</Info>
...
<AccordionGroup>
    <Accordion title="Azure">
        KMS offers a dedicated **Azure** provider to stream your audit logs, enabling seamless integration with services like Microsoft Sentinel.
        ...
        <Steps>
            ...
            <Step title="Create a Custom Log Table">
                ...
                Configure the Custom Log Table: Provide a **Table name** (e.g., `KMSLogs`), select the **Data collection endpoint** created in Step 1, and create a new **Data collection rule** as illustrated in the image below. Then, click **Next**.
            ...
            <Step title="Create Audit Log Stream on KMS">
                In KMS, create a new audit log stream and select the **Azure** provider. Input the following details:
    ...
    <Accordion title="Better Stack">
        ...
        <Steps>
            ...
            <Step title="Create Audit Log Stream on KMS">
        				On KMS, create a new audit log stream and select the **Custom** option.
    ...
    <Accordion title="Cribl">
        Stream KMS audit logs to Cribl Stream for centralized processing and routing. KMS supports Cribl as a provider for seamless integration.

        <Steps>
            <Step title="Create KMS Data Source">
                ...
                Configure the **Input ID**, **Port**, and **Cribl HTTP event API** path (e.g., `/kms`). Then, generate an **Auth Token**.
            ...
            <Step title="Create Audit Log Stream on KMS">
                On KMS, create a new audit log stream and select the **Cribl** provider option.
    ...
    <Accordion title="Datadog">
        ...
        <Steps>
            ...
            <Step title="Create Audit Log Stream on KMS">
                On KMS, create a new audit log stream and select the **Datadog** provider option.
    ...
    <Accordion title="Splunk">
        ...
        <Steps>
            ...
            <Step title="Create Audit Log Stream on KMS">
                On KMS, create a new audit log stream and select the **Splunk** provider option.
...
```created-secret.json
{
  ...
  "actor": "user",
  "actorMetadata": {
    "email": "example@kms.hanzo.ai",
    "userId": "7383b701-d83f-45c0-acb4-04e138b987ab",
    "username": "example@kms.hanzo.ai"
  },
  "ipAddress": "127.0.0.1",
