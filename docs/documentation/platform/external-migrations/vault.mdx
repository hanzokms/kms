...
sidebarTitle: "Vault"
description: "Learn how to migrate resources from Vault to KMS."
---

This guide helps you migrate from HashiCorp Vault to KMS. Whether you're moving a single application or your entire secrets infrastructure, KMS provides tooling to make the transition smooth.

Our in-platform migration tooling lets you set up a connection to your Vault instance once, then import specific resources as needed throughout KMS. Think of it as creating a "bridge" between Vault and KMS that you can use repeatedly.

**What you can migrate:**
- KV v1 and KV v2 secrets
- Kubernetes auth method configurations (become [Machine Identities](/documentation/platform/identities/machine-identities) in KMS)
- Kubernetes secrets engine roles (become [Dynamic Secrets](/documentation/platform/dynamic-secrets/kubernetes) in KMS)
- Vault policies (translated to [KMS project roles](/documentation/platform/access-controls/role-based-access-controls))
...
Before importing anything, you need to establish a secure connection between KMS and your Vault instance. This is a one-time setup.

<Steps>
  <Step title="Create a Vault policy">
    In your Vault instance, create a policy that allows KMS to read your secrets, policies, and authentication configurations. This policy grants read-only access and doesn't allow KMS to modify or create resources within your Vault instance.
  ...
  <Step title="Create an App Connection in KMS">
    In KMS, navigate to **Organization Settings > App Connections** and create a new HashiCorp Vault connection.
  ...
  <Step title="Add Vault Namespaces for Migration">
    Navigate to **Organization Settings > External Migrations** in KMS.
...
Once your Vault connection is configured, you'll see import options throughout KMS wherever relevant. The sections below cover each type of resource you can migrate.
...
<Info>
**First time?** You'll need to [create a project](/documentation/platform/project) first. A project in KMS typically represents a single application or microservice.
</Info>
...
The secrets will be imported into your current environment and folder path. If your Vault path has nested paths, you can create [folders](/documentation/platform/folder) in KMS to maintain a similar structure.

### Migrating Kubernetes Auth Method

If you're using Vault's Kubernetes auth method to authenticate your pods, you can migrate this configuration to KMS's [Machine Identities](/documentation/platform/identities/machine-identities). In Vault, you define Kubernetes roles that specify which service accounts can authenticate. In KMS, this maps to a Machine Identity with Kubernetes authentication enabled.

**To import Kubernetes auth configuration:**

1. First, [create a Machine Identity](/documentation/platform/identities/machine-identities) in KMS
...
**After importing:** You'll need to update your Kubernetes deployments to use the [KMS Operator](/integrations/platforms/kubernetes/overview) or [KMS SDK](/sdks/overview) instead of the Vault agent/sidecar.

### Migrating Kubernetes Secrets Engine Roles

If you're using Vault's Kubernetes secrets engine to generate short-lived Kubernetes credentials, you can migrate these roles to KMS's [Dynamic Secrets](/documentation/platform/dynamic-secrets/kubernetes). Both platforms support generating temporary Kubernetes service account tokens or creating temporary service accounts, so the configuration is similar.
...
Learn more about how Dynamic Secrets work in KMS in the [Dynamic Secrets documentation](/documentation/platform/dynamic-secrets/overview).

### Migrating Vault Policies

KMS can translate your Vault HCL policies into [KMS project roles](/documentation/platform/access-controls/role-based-access-controls). The table below shows how Vault capabilities map to KMS permissions:

| Vault Capability | KMS Permission |
|-----------------|---------------------|
| `read` | Read secrets |
...
<Note>
  ...
  adjusted as needed since Vault and KMS have different access control
  models.
</Note>
...
5. Review the automatically translated KMS permissions
...
<Tip>
  **How policy translation works:**
  ...
  - Vault capabilities (`read`, `list`, `create`, etc.) are mapped to KMS permissions
  ...
  **Example translation:**
  ...
  Becomes an KMS role with **read** permission on secrets in the corresponding environment/folder path.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Update Your Applications" icon="code" href="/sdks/overview">
    Replace Vault client libraries with KMS SDKs in your application code
  </Card>
  <Card title="Set Up Kubernetes Integration" icon="dharmachakra" href="/integrations/platforms/kubernetes/overview">
    Use the KMS Operator to inject secrets into your pods
  </Card>
</CardGroup>
