...
The KMS Azure SQL Database dynamic secret allows you to generate Azure SQL Database user credentials on demand based on configured roles.
...
When creating dynamic credentials for Azure SQL Database, KMS performs a two-step process:
...
When a lease is revoked or expires, KMS automatically:
