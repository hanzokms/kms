...
description: "Learn about the architecture, components, and security model of KMS PAM."
---

KMS PAM utilizes a secure, proxy-based architecture designed to provide access to private resources without exposing them directly to the internet. This system relies on a combination of the KMS CLI, a Relay server, and a self-hosted Gateway. For more information on Gateways, refer to the [Gateway Overview](/documentation/platform/gateways/overview).
...
When a user accesses a resource (e.g., via `kms access`), the following workflow occurs:

1. **Connection Initiation**: The KMS CLI initiates a connection to the Relay server.
...
- **Upload**: Once the session concludes, the logs are securely uploaded to the KMS platform for storage and review.
...
The connection between the KMS CLI (client) and the Gateway is end-to-end encrypted. The Relay server acts solely as a router for encrypted packets and **cannot decrypt or inspect** the traffic passing through it.
...
- **Outbound-Only**: The Gateway only requires outbound connectivity to the Relay server and KMS API.
