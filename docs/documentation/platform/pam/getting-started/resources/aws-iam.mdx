...
description: "Learn how to configure AWS Management Console access through KMS PAM for secure, audited, and just-in-time access to AWS."
---
...
Unlike database or SSH resources that require a Gateway for network connectivity, AWS Console access works differently. KMS uses AWS STS (Security Token Service) to assume roles on your behalf and generates temporary federated sign-in URLs.
...
sequenceDiagram
    ...
    User->>KMS: Request AWS Console access
    KMS->>Resource Role: AssumeRole (with ExternalId)
    Resource Role-->>KMS: Temporary credentials
    KMS->>Target Role: AssumeRole (role chaining)
    Target Role-->>KMS: Session credentials
    KMS->>Console: Generate federation URL
    Console-->>KMS: Signed console URL
    KMS-->>User: Return console URL
    User->>Console: Open AWS Console (federated)
```

### Key Concepts

1. **Resource Role**: An IAM role in your AWS account that trusts KMS. This is the "bridge" role that KMS assumes first.
...
3. **Role Chaining**: KMS uses AWS role chaining - it first assumes the Resource Role, then uses those credentials to assume the Target Role. This provides an additional layer of security and audit capability.

4. **External ID**: A unique identifier (your KMS Project ID) used in the trust policy to prevent [confused deputy attacks](https://docs.aws.amazon.com/IAM/latest/UserGuide/confused-deputy.html).
...
This allows you to correlate KMS PAM sessions with CloudTrail logs for complete audit visibility.

## Prerequisites

Before configuring AWS Console access in KMS PAM, you need to set up two IAM roles in your AWS account:

1. **Resource Role** - Trusted by KMS, can assume target roles
2. **Target Role(s)** - The actual roles users will use in the console

<Info>
  **No Gateway Required**: Unlike database or SSH resources, AWS Console access does not require an KMS Gateway. KMS communicates directly with AWS APIs.
</Info>

## Create the PAM Resource

The PAM Resource represents the connection between KMS and your AWS account. It contains the Resource Role that KMS will assume.
...
<Steps>
  <Step title="Create the Resource Role Permissions Policy">
    ...
    <Note>
      **For more granular control**: If you want to restrict which roles the Resource Role can assume, replace the wildcard (`/*`) with a more specific pattern. For example:
      ...
      - `arn:aws:iam::<YOUR_ACCOUNT_ID>:role/kms-*` to only allow roles with the `kms-` prefix
  ...
  <Step title="Create the Resource Role with Trust Policy">
    Create an IAM role (e.g., `KMSResourceRole`) with:
    ...
    <Warning>
      **Security Best Practice**: Always use the External ID condition. This prevents confused deputy attacks where another KMS customer could potentially trick KMS into assuming your role.
    </Warning>
    ...
    **KMS AWS Account IDs:**
    ...
    <Note>
      **For Dedicated Instances**: Your AWS account ID differs from the ones listed above. Please contact KMS support to obtain your dedicated AWS account ID.
    </Note>

    <Note>
      **For Self-Hosted Instances**: Use the AWS account ID where your KMS instance is deployed. This is the account that hosts your KMS infrastructure and will be assuming the Resource Role.
    </Note>
  </Step>

  <Step title="Create the Resource in KMS">
    ...
    Clicking **Create Resource** will validate that KMS can assume the Resource Role. If the connection fails, verify:
    - The trust policy has the correct KMS AWS account ID
...
<Steps>
  <Step title="Create the Target Role Trust Policy">
    ...
    ```json
    {
      ...
      "Statement": [{
        "Effect": "Allow",
        "Principal": {
          "AWS": "arn:aws:iam::<YOUR_ACCOUNT_ID>:role/KMSResourceRole"
        },
        "Action": "sts:AssumeRole",
        "Condition": {
          "StringEquals": {
  ...
  <Step title="Create the Account in KMS">
...
Once your resource and accounts are configured, users can request access through KMS:
