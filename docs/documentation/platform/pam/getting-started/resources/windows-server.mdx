...
description: "Learn how to configure Windows Server access through KMS PAM for secure, audited, and just-in-time access to your Windows servers via RDP."
---

KMS PAM supports secure, just-in-time access to Windows servers via the Remote Desktop Protocol (RDP).
...
Windows Server access in KMS PAM uses an KMS Gateway to securely proxy RDP connections to your Windows server. When a user requests access, KMS establishes a secure tunnel through the Gateway, enabling access without exposing your server directly.
...
1. **Gateway**: An KMS Gateway deployed in your network that can reach the Windows server. The Gateway handles secure communication between users and your server.

2. **RDP Protocol**: KMS connects to your Windows server using the Remote Desktop Protocol (RDP) on the configured port (default: 3389).
...
1. **KMS Gateway** - A Gateway deployed in your network with access to the Windows server
...
<Warning>
  **Gateway Required**: Windows Server access requires an KMS Gateway to be deployed and registered with your KMS instance. The Gateway must have network connectivity to your Windows server on the RDP port (default: 3389).
</Warning>
...
<Steps>
  <Step title="Ensure Gateway is Running">
    Before creating the resource, ensure you have an KMS Gateway running and registered with your KMS instance. The Gateway must have network access to your Windows server.
  </Step>
  ...
  <Step title="Create the Resource in KMS">
    ...
    Clicking **Create Resource** will trigger a connection test from the selected Gateway to your Windows server. KMS verifies that the server is reachable and responds to RDP connection requests.
  </Step>
</Steps>
