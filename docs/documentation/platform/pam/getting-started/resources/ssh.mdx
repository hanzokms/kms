...
title: "SSH"
sidebarTitle: "SSH"
description: "Learn how to configure SSH server access through KMS PAM with support for password, key-based, and certificate-based authentication."
---
...
SSH access in KMS PAM uses a Gateway deployed in your network to establish secure connections to your servers. The Gateway acts as a bridge between KMS and your private infrastructure.
...
sequenceDiagram
    ...
    CLI->>KMS: Request session (authenticate)
    KMS-->>CLI: Session ID + Gateway connection details
    CLI->>Gateway: Connect via secure tunnel
    Gateway->>KMS: Fetch credentials for session
    KMS-->>Gateway: Return credentials
...
Certificate-based authentication allows for the use of ephemeral credentials. KMS generates short-lived SSH certificates on-demand for each session.

**Benefits:**
- **Ephemeral access**: Certificates are generated per-session and expire automatically
- **Centralized trust**: Servers trust the KMS CA to authorize connections
- **Simplified management**: KMS handles the certificate lifecycle for each connection
...
sequenceDiagram
    ...
    CLI->>Gateway: Connect via secure tunnel
    Gateway->>KMS: Fetch credentials for session
    KMS->>KMS: Generate key pair + sign certificate
    KMS-->>Gateway: Return private key + certificate
...
<Steps>
  ...
  <Step title="Create the Resource">
    Click **Create Resource**. KMS will validate that the Gateway can reach the SSH server.
  </Step>
</Steps>
...
Certificate authentication requires additional setup on your SSH server to trust the KMS CA.

<Steps>
  <Step title="Configure the SSH Server">
    Before creating the account, you need to configure your SSH server to trust certificates signed by KMS.

    Run the following command on your SSH server (requires root/sudo):

    ```bash
    curl -H "Authorization: Bearer <YOUR_TOKEN>" \
      "https://kms.hanzo.ai/api/v1/pam/resources/ssh/<RESOURCE_ID>/ssh-ca-setup" \
      | sudo bash
    ```
    ...
    <Warning>
      **Manual setup**: If you prefer to configure the server manually, you can download just the CA public key and configure sshd yourself:
      ...
      ```bash
      curl -H "Authorization: Bearer <YOUR_TOKEN>" \
        "https://kms.hanzo.ai/api/v1/pam/resources/ssh/<RESOURCE_ID>/ssh-ca-public-key" \
  ...
  <Step title="Add Account in KMS">
    ...
    <Note>
      Unlike password or key authentication, you don't need to provide any credentials. KMS will generate a new key pair and certificate for each session.
    </Note>
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Connect to SSH Server">
    Specify the **resource name** and **account name** as shown in KMS:
