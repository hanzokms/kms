...
title: "Setup"
sidebarTitle: "Setup"
description: "This guide provides a step-by-step walkthrough for configuring KMS's Privileged Access Management (PAM). Learn how to deploy a gateway, define resources, and grant your team secure, audited access to critical infrastructure."
---

KMS's Privileged Access Management (PAM) solution enables you to provide developers with secure, just-in-time access to your critical infrastructure, such as databases, servers, and web applications. Instead of sharing static credentials, your team can request temporary access through KMS, which is then brokered through a secure gateway with full auditing and session recording.
...
- **Accounts:** The privileged credentials (e.g., a database user or an SSH user) that KMS uses to connect to a resource on behalf of a user.
...
<Steps>
  ...
  <Step title="Create a Resource">
    Once the Gateway is active, define a **Resource** in KMS (e.g., "Production Database"). You will link this resource to your deployed Gateway so KMS knows how to reach it.
