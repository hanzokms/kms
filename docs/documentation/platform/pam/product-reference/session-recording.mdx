...
description: "Learn how KMS records and stores session activity for auditing and monitoring."
---
...
Once the session concludes, the gateway transmits the complete recording to the KMS platform for long-term, centralized storage. This asynchronous process ensures that sessions remain operational even if the connection to the KMS platform is temporarily lost. After the upload is complete, administrators can search and review the session logs on the KMS platform.
...
<AccordionGroup>
    <Accordion title="Database Sessions">
        KMS captures all queries executed and their corresponding responses, including timestamps for each action.
    </Accordion>
    <Accordion title="SSH Sessions">
        KMS captures all commands executed and their corresponding responses, including timestamps for each action.
    </Accordion>
</AccordionGroup>
...
<AccordionGroup>
    ...
    <Accordion title="Why aren't recordings streamed in real-time?">
        Currently, KMS uses an asynchronous approach where the gateway records the entire session locally before uploading it. This design makes your PAM sessions more resilient, as they don't depend on a constant, active connection to the KMS platform. We may introduce live streaming capabilities in a future release.
    </Accordion>
</AccordionGroup>
