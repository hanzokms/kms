...
<Note>
  ...
  sales@kms.hanzo.ai if you have any questions.
</Note>

KMS KMS provides Key Management Interoperability Protocol (KMIP) support for integration with KMIP-compatible clients. This allows for enhanced key management across various applications that support the KMIP 1.4 protocol.

## How KMIP Works with KMS
...
2. The KMIP server acts as a proxy and forwards requests to KMS KMS
3. The KMIP server authenticates to KMS using a machine identity

The KMIP server itself is deployed using the KMS CLI (`kms kmip start` command) and serves as an intermediary between your KMIP clients and KMS's key management system.
...
- **KMIP Server → KMS Platform**: The KMIP server needs outbound HTTP access to KMS. For self-hosted instances, ensure connectivity to your custom domain.

## Configure and Deploy the KMIP Server

Follow these steps in order to set up KMIP integration with KMS:

<Steps>
  <Step title="Enable KMIP at the Organization Level">
    First, you need to enable KMIP for your entire KMS organization and set up its PKI infrastructure.
    ...
    <Info>
    You do not need to manage these certificates yourself; KMS handles the PKI infrastructure for you.
    </Info>
  </Step>

  <Step title="Create a Machine Identity for the KMIP Server">
    The KMIP server needs a machine identity to authenticate with KMS and proxy requests on behalf of clients.
    ...
    This machine identity will be used by the KMIP server to authenticate and forward client requests to KMS KMS.
  </Step>
  ...
  <Step title="Create a Custom Organization Role with Proxy KMIP Permission">
    ...
    This permission allows the KMIP server to act as an intermediary between KMIP clients and KMS.
  </Step>
  ...
  <Step title="Assign the Organization Role to the Machine Identity">
    ...
    This grants the machine identity the ability to serve KMIP client requests and forward them from your KMIP server to KMS.
  </Step>

  <Step title="Deploy the KMIP Server">
    Now you're ready to deploy the KMIP server. 
    You can run the KMIP server on any infrastructure that can reach the KMS platform, such as a VM or container. 
    ...
    To install the latest KMS CLI visit [KMS CLI instructions](https://kms.hanzo.ai/docs/cli/overview). 
    If you need to install specific versions of the CLI, you can find them on the [KMS CLI GitHub Releases](https://github.com/KMS/cli/releases).
    ...
    **Available flags:**
    ...
    - **identity-client-id**: The client ID of the machine identity (can be set via `KMS_UNIVERSAL_AUTH_CLIENT_ID` env var)
    - **identity-client-secret**: The client secret of the machine identity (can be set via `KMS_UNIVERSAL_AUTH_CLIENT_SECRET` env var)
    ...
    Once started, your KMIP server is now running and ready to accept client connections. It will authenticate to KMS using the machine identity and proxy all KMIP operations.
  </Step>
  ...
  <Step title="Configure the KMIP Client">
    ...
    This creates a KMIP client entity in KMS that will be authenticated via mTLS certificates.
  </Step>
  ...
  <Step title="Generate Client Certificate">
    ...
    Configure your KMIP-compatible applications or tools to use these credentials when connecting to the KMIP server. The client will now authenticate via mTLS and perform authorized key management operations through the KMIP server, which proxies requests to KMS KMS.
  </Step>
</Steps>

## Connecting your KMIP Client to KMS
...
2. **Client Certificates**: The certificate credentials generated from your KMS KMS project:
...
<Steps>
  ...
  <Step title="Prepare Certificate Files">
    Organize the certificate materials you downloaded when generating the client certificate from the KMS KMS project. You should have three files:
    ...
    Most KMIP clients require these files in PEM format, which is what KMS provides by default.
  </Step>
  ...
  <Step title="Test the Connection">
    ...
    If the connection is successful, your KMIP client is now integrated with KMS KMS and can perform key management operations according to the permissions you assigned.
