...
description: "Learn how to configure an AWS Elastic Load Balancer Certificate Sync for KMS PKI."
---
...
<Note>
  The AWS Elastic Load Balancer Certificate Sync requires the following IAM permissions for KMS to sync certificates to your load balancers:
  ...
  **ELB Permissions:** `elasticloadbalancing:DescribeLoadBalancers`, `elasticloadbalancing:DescribeListeners`, `elasticloadbalancing:DescribeListenerCertificates`, `elasticloadbalancing:AddListenerCertificates`, `elasticloadbalancing:RemoveListenerCertificates`, `elasticloadbalancing:ModifyListener`
  ...
  These permissions allow KMS to import certificates to ACM and attach them to your load balancer listeners.
</Note>
...
<Tabs>
    <Tab title="KMS UI">
        ...
        - **Enable Removal of Expired/Revoked Certificates**: If enabled, KMS will remove certificates from listeners and delete them from ACM when they are no longer active in KMS.
        - **Preserve ARN on Renewal**: If enabled, KMS will sync renewed certificates to ACM under the same ARN as the original synced certificate instead of creating a new certificate with a new ARN.
        ...
        - **Certificate Name Schema** (Optional): Customize how certificate tags are generated in ACM. Must include `{{certificateId}}` as a placeholder for the certificate ID to ensure proper certificate identification and management. If not specified, defaults to `KMS-{{certificateId}}`.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://kms.hanzo.ai/api/v1/cert-manager/syncs/aws-elastic-load-balancer \
...
- **Tagging**: Automatically tag certificates in ACM with an KMSCertificate tag for easy identification and management.
...
<Tabs>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://kms.hanzo.ai/api/v1/cert-manager/syncs/aws-elastic-load-balancer/{pkiSyncId}/certificates/default \
        --header 'Authorization: Bearer <access-token>' \
        --header 'Content-Type: application/json' \
        --data '{
...
To manually sync certificates, use the [Sync Certificates](/api-reference/endpoints/pki/syncs/aws-elastic-load-balancer/sync-certificates) API endpoint or the manual sync option in the KMS UI.
...
<Note>
  ...
  from AWS Certificate Manager. Only certificates managed by KMS can be
