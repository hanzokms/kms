...
description: "Learn how to configure a Cloudflare Custom Certificate Sync for KMS PKI."
---
...
<Tabs>
    <Tab title="KMS UI">
        ...
        ![Select Cloudflare](/images/platform/pki/certificate-syncs/cloudflare-custom-certificate/select-cloudflare-option.png)

        3. Configure the **Destination** to where certificates should be deployed, then click **Next**.
        ![Configure Destination](/images/platform/pki/certificate-syncs/cloudflare-custom-certificate/cloudflare-destination.png)
        ...
        ![Configure Options](/images/platform/pki/certificate-syncs/cloudflare-custom-certificate/cloudflare-options.png)

        - **Enable Removal of Expired/Revoked Certificates**: If enabled, KMS will remove certificates from the destination if they are no longer active in KMS.
        - **Certificate Name Schema** (Optional): Customize how certificate names are generated. Must include `{{certificateId}}` as a placeholder. If not specified, defaults to `KMS-{{certificateId}}`.
        ...
        ![Configure Details](/images/platform/pki/certificate-syncs/cloudflare-custom-certificate/cloudflare-details.png)

            - **Name**: The name of your sync. Must be slug-friendly.
            - **Description**: An optional description for your sync.

        6. Select which certificates should be synced to Cloudflare.
        ![Select Certificates](/images/platform/pki/certificate-syncs/cloudflare-custom-certificate/cloudflare-certificates.png)

        7. Review your Cloudflare Custom Certificate Sync configuration, then click **Create Sync**.
        ![Confirm Configuration](/images/platform/pki/certificate-syncs/cloudflare-custom-certificate/cloudflare-review.png)

        8. If enabled, your Cloudflare Custom Certificate Sync will begin syncing your certificates to the destination endpoint.
        ![Sync Certificates](/images/platform/pki/certificate-syncs/cloudflare-custom-certificate/cloudflare-synced.png)
    </Tab>
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://kms.hanzo.ai/api/v1/cert-manager/syncs/cloudflare-custom-certificate \
...
- **Automatic Deployment**: Deploy certificates in KMS to Cloudflare as Custom certificates.
...
To manually sync certificates, use the [Sync Certificates](/api-reference/endpoints/pki/syncs/cloudflare-custom-certificate/sync-certificates) API endpoint or the manual sync option in the KMS UI.

## FAQ

<Accordion title="Can I import certificates from Cloudflare back into KMS?">
  Cloudflare does not support importing certificates back into KMS
