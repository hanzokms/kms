...
description: "Learn how to sync certificates from KMS PKI to third-party services."
---

Certificate Syncs enable you to push certificates from KMS to third-party services using [App Connections](/integrations/app-connections/overview).
...
Certificate Syncs are a project-level resource used to push certificates, via an [App Connection](/integrations/app-connections/overview), from KMS
...
<Note>
  Only certificates managed by KMS will be affected during sync
  operations. Certificates not created or managed by KMS will remain
  untouched, and changes made to KMS-managed certificates directly in the
...
<Note>
  KMS is continuously expanding its Certificate Sync third-party service
  support. If the service you need isn't available, contact us at
  team@kms.hanzo.ai to make a request.
</Note>
...
By default, certificates are named using the pattern `KMS-{certificateId}` where `{certificateId}` is the unique identifier of the certificate with hyphens removed for compatibility with services like Azure Key Vault.
...
- Certificate names matching your schema will be managed by KMS during sync operations
