...
The API enrollment method allows you to issue certificates against a specific [certificate profile](/documentation/platform/pki/certificates/profiles) over Web UI or by making an API request to KMS.
...
<Steps>
  <Step title="Create a certificate profile in KMS">
    ...
    ```bash Request
    curl --location --request POST 'https://kms.hanzo.ai/api/v1/cert-manager/certificate-profiles' \
