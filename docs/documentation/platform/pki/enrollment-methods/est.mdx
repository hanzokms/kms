...
and structured under `https://kms.hanzo.ai:8443/.well-known/est/{profile_id}/...`

## Prerequisites

- Your client devices need to have a bootstrap/pre-installed certificate.
- Your client devices must trust the server certificates used by KMS's EST server. If the devices are new or lack existing trust configurations, you need to manually establish trust for the appropriate certificates.

<Note>
  For KMS Cloud users, the devices must be configured to trust the [Amazon
...
<Steps>
  ...
  ![pki est config](/images/platform/pki/enrollment-methods/est/est-config.png)
    ...
    - EST Passphrase: This is also used to authenticate your devices with KMS's EST server. When configuring the clients, use the value defined here as the EST password.
    - CA Chain Certificate: This is the certificate chain used to validate your devices' manufacturing/pre-installed certificates. This will be used to authenticate your devices with KMS's EST server.
  ...
  <Step title="Obtain the EST label">
    ...
    `profile_id` as the EST label when enrolling EST clients with KMS.
    ...
    - https://kms.hanzo.ai:8443/.well-known/est/{profile_id}/cacerts
    - https://kms.hanzo.ai:8443/.well-known/est/{profile_id}/simpleenroll
    - https://kms.hanzo.ai:8443/.well-known/est/{profile_id}/simplereenroll
    </Step>
