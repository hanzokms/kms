...
description: "Learn how to set up webhook notifications for PKI certificate alerts"
---

KMS can send PKI certificate alert notifications to a webhook URL. This guide walks through creating a webhook alert, the payload format, and how to verify webhook signatures.
...
Next, add a **Webhook** notification channel. The URL must use HTTPS. Optionally configure a signing secret to verify the authenticity of webhook payloads.
...
{
  "specversion": "1.0",
  "type": "com.kms.pki.certificate.expiration",
  ...
  "data": {
    ...
    "metadata": {
      "totalCertificates": 1,
      "viewUrl": "https://kms.hanzo.ai/cert-manager/<project-id>/policies"
    }
  }
}
```
...
If you configure a signing secret for your webhook channel, KMS will include an `x-kms-signature` header with each request. This allows you to verify that the webhook payload originated from KMS.

The header format is:

```
x-kms-signature: t=<timestamp>,v1=<signature>
```
