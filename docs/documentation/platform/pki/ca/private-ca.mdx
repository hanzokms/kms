...
KMS lets you build your Internal PKI through a Private Certificate Authority (CA) hierarchy, enabling you to issue and manage digital certificates for your end-entities.
...
<Steps>
  <Step title="Creating a root CA">
    ...
    ```bash Request
    curl --location --request POST 'https://kms.hanzo.ai/api/v1/cert-manager/ca/internal' \
  ...
  <Step title="Creating an intermediate CA">
    ...
    ```bash Request
    curl --location --request GET 'https://kms.hanzo.ai/api/v1/cert-manager/ca/internal/<intermediate-ca-id>/csr' \
      --header 'Authorization: Bearer <access-token>' \
      --data-raw ''
    ```
    ...
    ```bash Request
    curl --location --request POST 'https://kms.hanzo.ai/api/v1/cert-manager/ca/internal/<root-ca-id>/sign-intermediate' \
      --header 'Content-Type: application/json' \
      --data-raw '{
          "csr": "<csr>",
          "notAfter": "2029-06-12"
    ...
    ```bash Request
    curl --location --request POST 'https://kms.hanzo.ai/api/v1/cert-manager/ca/internal/<intermediate-ca-id>/import-certificate' \
      --header 'Authorization: Bearer <access-token>' \
      --header 'Content-Type: application/json' \
      --data-raw '{
...
<Tabs>
  ...
  To renew a CA certificate, make an API request to the [Renew CA](/api-reference/endpoints/certificate-authorities/renew) API endpoint, specifying the new `notAfter` date for the CA.
    ...
    ```bash Request
    curl --location --request POST 'https://kms.hanzo.ai/api/v1/cert-manager/ca/internal/<ca-id>/renew' \
      --header 'Authorization: Bearer <access-token>' \
      --header 'Content-Type: application/json' \
      --data-raw '{
          "type": "existing",
