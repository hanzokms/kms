...
description: "Learn how to automatically provision and manage TLS certificates in Kubernetes using KMS"
---

## Concept

This guide demonstrates how to use KMS to issue TLS certificates back to your Kubernetes environment using [cert-manager](https://cert-manager.io/).

It uses the [ACME issuer type](https://cert-manager.io/docs/configuration/acme/) to request and renew certificates automatically from KMS
...
The typical workflow involves installing `cert-manager` and configuring resources that represent the connection details to KMS as well as the certificates you want to issue.
...
A typical workflow for using cert-manager with KMS via ACME consists of the following steps:

1. Create a [certificate profile](/documentation/platform/pki/certificates/profiles) in KMS with the [ACME enrollment method](/documentation/platform/pki/enrollment-methods/acme) configured on it.
...
4. Create an `Issuer` or `ClusterIssuer` resource that connects to the desired KMS [certificate profile](/documentation/platform/pki/certificates/profiles).
...
The following steps show how to install cert-manager (using `kubectl`) and obtain certificates from KMS.

<Steps>
  <Step title="Create a certificate profile with ACME as the enrollment method in KMS">
    ...
    <Note>
      Currently, the KMS ACME enrollment method only supports authentication via dedicated EAB credentials generated per certificate profile.
  ...
  <Step title="Create a Kubernetes Secret for the KMS ACME EAB credentials">
    Create a Kubernetes `Secret` that contains the **EAB Secret (HMAC key)** obtained in step 1.
    The cert-manager uses this secret to authenticate with the KMS ACME server.
  ...
  <Step title="Create the cert-manager Issuer connecting to KMS ACME server">
    ...
    spec:
        acme:
            # ACME server URL from your KMS certificate profile (Step 1)
            server: <acme_server_url>
            # Email address for ACME account
            # (any valid email works; currently ignored by KMS)
            email: <your_email>
    ...
    ```
    kubectl apply -f issuer-kms.yaml
    ```
  ...
  <Step title="Create the Certificate">
    ...
    Finally, request a certificate from KMS ACME server by creating a cert-manager `Certificate` resource.
    ...
    <Info>
      cert-manager does not currently support specifying a `pathLen` in the Certificate resource. When issuing CA certificates with `isCA: true`, ensure your KMS certificate policy does not set a **Maximum Allowed Path Length** restriction, otherwise the request will fail validation.
    </Info>
...
Create a Kubernetes Secret containing your CA certificate chain. To obtain the certificate chain from KMS:

1. Navigate to your project in KMS
...
```bash
kubectl apply -f kms-ca-cert.yaml
```
