---
title: "Tomcat"
description: "Learn how to issue TLS certificates from KMS using ACME enrollment on Tomcat with Certbot"
---

This guide demonstrates how to use KMS to issue TLS certificates for your [Apache Tomcat](https://tomcat.apache.org/) application server.

It uses [Certbot](https://certbot.eff.org/), an installable [ACME](https://en.wikipedia.org/wiki/Automatic_Certificate_Management_Environment) client, to request and renew certificates from KMS using the [ACME enrollment method](/documentation/platform/pki/enrollment-methods/acme) configured on a [certificate profile](/documentation/platform/pki/certificates/profiles). Unlike web servers with native Certbot plugins, Tomcat requires certificates to be manually configured after issuance.
...
- Network connectivity from your Tomcat server to KMS.
...
<Steps>
    <Step title="Obtain ACME Configuration from KMS">
        ...
        - ACME Directory URL: The URL that Certbot will use to communicate with KMS's ACME server. This takes the form `https://your-kms-instance.com/api/v1/cert-manager/certificate-profiles/{profile-id}/acme/directory`.
    ...
    <Step title="Request Certificate Using Certbot">
        ...
        Run the following command to request a certificate from KMS:
        ...
        - `--server`: The KMS ACME directory URL from Step 1. This instructs Certbot to communicate with KMS's ACME server instead of Let's Encrypt.
        ...
        The Certbot command generates a private key on your server, creates a Certificate Signing Request (CSR) using that key, and sends the CSR to KMS for certificate issuance. Certbot stores the private key and resulting leaf certificate and full certificate chain in `/etc/letsencrypt/live/{domain-name}/`.
    ...
    <Step title="Configure Tomcat SSL Connector">
        ...
        <Connector port="8443"
                   ...
                   SSLEnabled="true">
            <SSLHostConfig>
                <Certificate certificateFile="/etc/letsencrypt/live/example.kms.hanzo.ai/cert.pem"
                           certificateKeyFile="/etc/letsencrypt/live/example.kms.hanzo.ai/privkey.pem"
                           certificateChainFile="/etc/letsencrypt/live/example.kms.hanzo.ai/chain.pem"
                           type="RSA" />
            </SSLHostConfig>
        </Connector>
        ```
        ...
        You can verify SSL is working by accessing your Tomcat application at `https://example.kms.hanzo.ai:8443`. For production deployments, consider configuring a reverse proxy (like [Apache HTTP Server](https://httpd.apache.org/) or [Nginx](https://nginx.org/)) to handle SSL termination on standard port 443.
    ...
    <Step title="Using Renewed Certificates Manually">
        ...
        **Step 1: Set certificate file permissions**
        ...
        ```bash
        sudo chown root:tomcat /etc/letsencrypt/live/example.kms.hanzo.ai/cert.pem
        sudo chown root:tomcat /etc/letsencrypt/live/example.kms.hanzo.ai/privkey.pem
        sudo chown root:tomcat /etc/letsencrypt/live/example.kms.hanzo.ai/chain.pem
        sudo chmod 640 /etc/letsencrypt/live/example.kms.hanzo.ai/cert.pem
        sudo chmod 640 /etc/letsencrypt/live/example.kms.hanzo.ai/privkey.pem
        sudo chmod 640 /etc/letsencrypt/live/example.kms.hanzo.ai/chain.pem
        ```
