---
title: "Gloo Mesh"
description: "Learn how to automatically provision and manage Istio intermediate CA certificates for Gloo Mesh using KMS"
---

This guide will provide a high level overview on how you can use KMS and [cert-manager](https://cert-manager.io/) to issue Istio intermediate CA certificates for your Gloo Mesh workload clusters. For more background about Istio certificates, see the [Istio CA overview](https://istio.io/latest/docs/concepts/security/#pki).

## Overview

In this setup, we will use KMS to generate and store your root CA and subordinate CAs that are used to generate Istio intermediate CAs for your Gloo Mesh workload clusters.
...
KMS verifies the request against your certificate policies and returns the signed certificate.
...
Follow the [Kubernetes cert-manager guide](/documentation/platform/pki/k8s-cert-manager) for detailed instructions on how to set up the KMS and cert-manager for your Istio intermediate CA certificates in Gloo Mesh clusters.
...
- Certificate profiles in KMS are configured for intermediate CA usage with appropriate key usage and constraints
- Multiple workload clusters use the same KMS root to enable cross-cluster mTLS communication
...
When you deploy applications to your Gloo Mesh service mesh, the workloads will receive leaf certificates signed by your KMS intermediate CA, enabling secure mTLS communication across your entire mesh infrastructure.
