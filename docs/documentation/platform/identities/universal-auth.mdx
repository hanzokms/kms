...
<Steps>
  <Step title="Creating an identity">
    ...
    **Advanced Tab**
    ...
    If you're using KMS Cloud, then it is available under the Pro Tier. If you're self-hosting KMS, then you should contact sales@kms.hanzo.ai to purchase an enterprise license to use it.
    </Warning>
  ...
  <Step title="Accessing the KMS API with the identity">
    ...
    <Tip>
      ...
      - For KMS Cloud US users: `https://kms.hanzo.ai`
      - For KMS Cloud EU users: `https://eu.kms.hanzo.ai`
    </Tip>

    #### Sample request

    ```bash Request
    curl --location --request POST 'https://kms.hanzo.ai/api/v1/auth/universal-auth/login' \
...
```bash
curl --location --request POST 'https://kms.hanzo.ai/api/v1/auth/universal-auth/renew' \
