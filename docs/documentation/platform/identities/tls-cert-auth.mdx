...
<Note>
  ...
  When you are authenticating with TLS Certificate Auth, you must use the port `8443` instead of the default `443`. Example: `https://kms.hanzo.ai:8443/api/v1/auth/tls-cert-auth/login`
</Note>
...
Since the identity has been configured with [Universal Auth](https://kms.hanzo.ai/docs/documentation/platform/identities/universal-auth) by default, you should reconfigure it to use TLS Certificate Auth instead. To do this, click the cog next to **Universal Auth** and then select **Delete** in the options dropdown.
...
try {
  ...
  const kmsUrl = "https://kms.hanzo.ai:8443"; // or your self-hosted KMS URL
