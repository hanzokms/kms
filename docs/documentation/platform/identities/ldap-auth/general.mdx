  ...
  <Step title="Configuring LDAP auth for your identity">
    ...
    **Advanced Tab**
    ...
    <Step title="Authenticating with the identity">
      ...
      ```bash
        curl --request POST \
          --url https://kms.hanzo.ai/api/v1/auth/ldap-auth/login \
      ...
      <Note>
        For EU Cloud and Self-Hosted users, make sure to replace `https://kms.hanzo.ai` with `https://eu.kms.hanzo.ai` or your self-hosted instance's URL in the request URL.
      </Note>
