...
<Steps>
    ...
    ACCESS_TOKEN=$(curl -s -X POST \
        ...
        https://kms.hanzo.ai/api/v1/auth/oidc-auth/login | jq -r '.accessToken')

    # Use the access token to retrieve secrets
    curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
        "https://kms.hanzo.ai/api/v3/secrets/raw?workspaceSlug=<project-slug>&environment=<env-slug>&secretPath=/"
    ```
