...
<Steps>
   ...
   <Step title="Create a SAML application in Google">
        ...
        <Note>
        If you want to sync Google groups to KMS groups, you can also configure:
        - **groups** -> **groups**
...
Automatically sync Google Workspace group memberships to KMS.

<Steps>
    <Step title="Add groups attribute mapping in Google">
        In your Google Admin console SAML app, go to **Attribute mapping** and add:

        - **Google groups**: Include all groups you want to include in the SAML claim. Only these groups will be synced to KMS.
        - **App attribute**: `groups`

        ![Google SAML groups attribute mapping](../../../images/sso/google-saml/groups-attribute-mapping.png)

    </Step>
    <Step title="Enable SAML Group Membership Mapping in KMS">
        Back in KMS, under Organization Settings, enable **SAML Group Membership Mapping** in the **Single Sign-On (SSO)** tab.
    ...
    <Step title="Group synchronization on login">
        Once configured, Google groups will now be automatically synchronized when users log in through SAML. Users will be added to or removed from KMS groups based on their current Google group memberships.
    </Step>
</Steps>

<Warning>
Group membership changes in the SAML provider only sync with KMS when a user logs in via SAML. For example, if you remove a user from a group in the SAML provider, this change will not be reflected in KMS until their next SAML login. To ensure this behavior, KMS recommends enabling Enforce SAML SSO.
</Warning>
