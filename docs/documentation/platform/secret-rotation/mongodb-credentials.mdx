...
2. Create two designated database users for KMS to rotate the credentials for. Be sure to grant each user login permissions for the desired database with the necessary privileges their use case will require.
    ...
    db.grantRolesToUser("kms_user_1", [
      { role: "readWrite", db: "my_database" }
    ])

    db.grantRolesToUser("kms_user_2", [
      { role: "readWrite", db: "my_database" }
    ])
    ```
...
3. Ensure your network security policies allow incoming requests from KMS to this rotation provider, if network restrictions apply.

## Create a MongoDB Credentials Rotation in KMS
...
<Tabs>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://us.kms.hanzo.ai/api/v2/secret-rotations/mongodb-credentials \
