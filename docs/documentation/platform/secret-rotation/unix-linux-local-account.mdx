...
<Note>
  **Rotation Type: [Single-Phase](/documentation/platform/secret-rotation/overview#single-phase-rotation)**
  ...
  This is a limitation of the SSH provider and cannot be rectified by KMS.
</Note>

## Prerequisites

- Create an [SSH Connection](/integrations/app-connections/ssh) with the **Secret Rotation** requirements
- Ensure your network security policies allow incoming requests from KMS to this rotation provider, if network restrictions apply.

## Create a Unix/Linux Local Account Rotation in KMS
...
<Tabs>
    <Tab title="KMS UI">
        ...
        - **Rotation Method** - The method to use when rotating the target user's password.
            - **Login as Target** - KMS will use the provided SSH username and password to log in and rotate its own password.
            - **Login as Root** - KMS will use the SSH Connection's credentials to log in and rotate the provided user's password.
        - **Username** - The target SSH username whose password will be rotated.
        - **Use Sudo** - If true, uses sudo when executing the password rotation command. Defaults to false.
        ...
        If you suspect the credentials are out of sync (for example, after a manual password change on the server), you can regain access by using **Reconcile**. This will use the configured SSH App Connection's root account to reset the target user's password and sync it with KMS.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://us.kms.hanzo.ai/api/v2/secret-rotations/unix-linux-local-account \
