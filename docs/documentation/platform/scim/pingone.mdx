---
title: "PingOne SCIM"
description: "Learn how to configure SCIM provisioning with PingOne for KMS."
---
...
<Info>
    ...
    then you should contact sales@kms.hanzo.ai to purchase a self-hosted license to use it.
</Info>

Prerequisites:
- [Configure PingOne OIDC for KMS](/documentation/platform/sso/pingone-oidc)
...
<Steps>
    ...
    <Step title="Configure SCIM Connection in PingOne">
        ...
        **2. Configure the connection**
        ...
        Once you have selected the SCIM Outbound option, you'll be prompted to enter the authentication details that PingOne will use to authenticate with KMS SCIM. This is the **SCIM URL** and **New SCIM Token** from the previous step.
    ...
    <Step title="Configure SCIM Provisioning in PingOne">
      ...
      **3. Configure the rule user filter**
      ...
      Select the Edit pencil icon to open the user filter configuration. This step dictates which users will be provisioned to KMS.
      ...
      **4. Configure Groups**
      ...
      This step is optional and only relevant if you want to provision PingOne groups to KMS.

      ![SCIM PingOne Create New Rule 6](/images/platform/scim/pingone/pingone-rule-group-provisioning-tab.png)

      Open the "Group Provisioning" tab and press the "Add Groups" button to select which groups will be provisioned to KMS.

      ![SCIM PingOne Create New Rule 7](/images/platform/scim/pingone/pingone-select-group.png)

      Select the groups you want to provision to KMS and press the "Save" button.
...
<Accordion title="Why do SCIM-provisioned users have to finish setting up their account?">
    KMS's SCIM implementation accounts for retaining the end-to-end encrypted architecture of KMS because we decouple the **authentication** and **decryption** steps in the platform.
