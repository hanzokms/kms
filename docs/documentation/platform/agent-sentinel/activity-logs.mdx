...
<AccordionGroup>
  ...
  <Accordion title="Are sensitive data in requests/responses masked?">
    KMS supports PII filtering to automatically detect and mask sensitive data in request and response payloads. When enabled on an MCP endpoint, the following PII types can be redacted:
