...
description: "Learn how to structure KMS for centralized platform administration or team self-service autonomy"
---

Organizations adopt different approaches to secrets management governance based on their security requirements, compliance obligations, and team structures. KMS supports a spectrum of governance models—from fully centralized platform administration to team-driven self-service.

This guide covers how to configure KMS for different governance approaches and what tradeoffs to consider.
...
KMS supports multiple authentication methods that can be configured based on your requirements:
...
[Machine identities](/documentation/platform/identities/machine-identities) authenticate applications, services, and automated systems with KMS. Your governance model shapes how these identities are managed:
...
KMS supports multiple machine identity authentication methods:
...
KMS's [role-based access control](/documentation/platform/access-controls/role-based-access-controls) operates at two levels: organization and project. How you configure these controls determines who can do what across your secrets infrastructure.
...
<Note>
Projects in KMS are isolated from one another. Secret referencing and imports work within a single project—you cannot reference secrets across different projects.
</Note>

## Integrations and Secret Delivery

KMS offers multiple methods for delivering secrets to applications and infrastructure.

### Secret Syncs

[Secret Syncs](/integrations/secret-syncs/overview) push secrets to third-party platforms like AWS Secrets Manager, Azure Key Vault, GCP Secret Manager, and others. Syncs keep external secret stores updated when values change in KMS.
...
- [KMS Operator](/integrations/platforms/kubernetes/kms-operator) — Syncs secrets to Kubernetes Secrets resources
- [KMS Secrets Injector](/integrations/platforms/kubernetes-injector) — Injects secrets directly into pods at runtime

| Approach | Centralized | Self-Service |
|----------|-------------|--------------|
| **Operator deployment** | Single cluster-wide instance managed by platform team | Teams may deploy namespace-scoped instances |
| **Secret sync patterns** | Standardized CRD configurations | Teams define their own KmsSecret resources |

### Agent and CLI

The [KMS Agent](/kms-agent/overview) and [CLI](/cli/overview) provide flexible secret consumption patterns:
...
[Gateways](/documentation/platform/gateways/overview) enable KMS to securely access private resources—such as databases in isolated VPCs—without exposing them to the public internet. Gateways are lightweight components deployed within your private network that establish secure, outbound-only connections to KMS.
...
Gateways operate at two levels within KMS:
...
Visibility into secrets access and changes is critical for security and compliance. KMS provides audit capabilities at both organization and project levels.
...
Beyond access control, KMS offers additional security settings.
...
- Training for teams on KMS features and security best practices
- Runbooks for common operations (secret rotation, access requests, incident response)

## Summary

Here's a quick reference for how key KMS features map to each governance model:
