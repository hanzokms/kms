...
If you're working with Rust, the official KMS Rust SDK package is the easiest way to fetch and work with secrets for your application.
...
```rust
use kms::{AuthMethod, Client, KMSError, encode_base64, decode_base64};
use kms::secrets::GetSecretRequest;

async fn fetch_secret() -> Result<(), KMSError> {
    // 1. Build the client. You can chain methods to configure it.
    let mut client = Client::builder()
        .base_url("https://kms.hanzo.ai") // Optional: defaults to https://kms.hanzo.ai
        .build()
        .await?;
...
```rust
let mut client = Client::builder()
    .base_url("https://kms.hanzo.ai") // Optional: set custom KMS instance URL
    .build()
    .await?;
```

**Parameters**
- `.base_url(url)`: Optional method to set the KMS instance URL. Defaults to `https://kms.hanzo.ai` for KMS Cloud. Use `https://eu.kms.hanzo.ai` for EU and `http://localhost:8080` for local development.
...
```rust
use kms::{encode_base64, decode_base64};
...
- `decode_base64(data: &str) -> Result<String, KMSError>`: Decodes a base64 string
...
```rust
use kms::secrets::CreateSecretRequest;
...
```rust
use kms::secrets::ListSecretsRequest;
...
```rust
use kms::secrets::UpdateSecretRequest;
...
```rust
use kms::secrets::DeleteSecretRequest;
...
```rust
use kms::kms::ListKmsKeysRequest;
...
```rust
use kms::kms::GetKmsKeyByNameRequest;
...
```rust
use kms::kms::{CreateKmsKeyRequest, EncryptionAlgorithm, KeyUsage};
...
```rust
use kms::kms::UpdateKmsKeyRequest;
...
```rust
use kms::kms::DeleteKmsKeyRequest;
...
```rust
use kms::kms::{SigningAlgorithm, SignRequest};
...
```rust
use kms::kms::{SigningAlgorithm, VerifyRequest};
...
1. **Client ID and Secret**: Create a Universal Auth machine identity in your KMS project settings
2. **Project ID**: Found in your project settings or URL when viewing a project in the KMS dashboard
...
**Note**: Integration tests require a running KMS instance and valid authentication credentials.