...
If you're working with Node.js, the official KMS Node.js SDK package is the easiest way to fetch and work with secrets for your application. 
...
const client = new KMSSDK({
  siteUrl: "your-kms-instance.com" // Optional, defaults to https://kms.hanzo.ai
});
...
- `viewSecretValue` (bool, optional): Whether or not to reveal the secret value of the secrets. If set to `false`, the `secretValue` is masked with `<hidden-by-kms>`. Defaults to `true`.
...
- `expandSecretReferences` (bool, optional): Whether to expand secret references.
- `viewSecretValue` (bool, optional): Whether or not to reveal the secret value of the secret. If set to `false`, the `secretValue` is masked with `<hidden-by-kms>`. Defaults to `true`.
...
For a more in-depth description of each input type for each dynamic secret type, please refer to [our API documentation](https://kms.hanzo.ai/docs/api-reference/endpoints/dynamic-secrets/create)


##### Example for creating a new Redis dynamic secret

```typescript
import { KMSSDK, DynamicSecretProviders } from "@hanzo/kms-sdk";
	const client = new KMSSDK();
...
- `kmsKeyId` (string): The ID of the KMS key to use for the project. Will use the KMS KMS by default.
...
- `position` (number): An optional position of the environment to be created. The position is used in the KMS UI to display environments in order. Environments with the lowest position come first.
...
```typescript
import { KMSSDK, KeyUsage, EncryptionAlgorithm } from "@hanzo/kms-sdk";
const client = new KMSSDK();
...
```typescript
import { SigningAlgorithm } from "@hanzo/kms-sdk";
