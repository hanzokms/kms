...
If you're working with Python, the official KMS Python SDK package is the easiest way to fetch and work with secrets for your application. 
...
1. **Removal of `rest`**: The SDK no longer exposes the entire KMS API. This was nessecary as we have moved away from using an OpenAPI generator approach. We aim to add support for more API resources in the near future. If you have any specific requests, please [open an issue](https://github.com/KMS/python-sdk-official/issues).

2. **New response types**: The 1.0.3 release uses return types that differ from the older versions. The new return types such as `BaseSecret`, are all exported from the KMS SDK.
...
You can refer to our [legacy documentation](https://github.com/KMS/python-sdk-official/tree/9b0403938ee5ae599d42c5f1fdf9158671a15606?tab=readme-ov-file#kms-python-sdk) if need be.
...
client = KMSSDKClient(host="https://kms.hanzo.ai")
...
# Use the SDK to interact with KMS
secrets = client.secrets.list_secrets(project_id="<project-id>", environment_slug="dev", secret_path="/")
```

## KMSSDKClient Parameters

The `KMSSDKClient` takes the following parameters, which are used as a global configuration for the lifetime of the SDK instance.

- **host** (`str`, _Optional_): The host URL for your KMS instance. Defaults to `https://kms.hanzo.ai`.
...
```python
client = KMSSDKClient(
  host="https://kms.hanzo.ai", # Defaults to https://kms.hanzo.ai
...
4. `kms`: Perform cryptographic operations with KMS KMS.
...
**Parameters:**
- `identity_id` (str): The ID of the OIDC identity configuration in KMS.
...
```python
response = client.auth.ldap_auth.login(
...
**Parameters:**
- `identity_id` (str): The ID of the LDAP identity configuration in Infisical.
...
- `view_secret_value` (bool): Whether or not to include the secret value in the response. If set to false, the `secretValue` will be masked with `<hidden-by-kms>`. Defaults to true.
...
```python
from kms_sdk import DynamicSecretProviders
...
- `path` (str, optional): The path to the dynamic secret. Defaults to "/".
- `is_forced` (bool, optional): A boolean flag to delete the dynamic secret from KMS without trying to remove it from the external provider. Defaults to `False`.
...
- `path` (str, optional): The path to the dynamic secret. Defaults to "/".
- `is_forced` (bool, optional): A boolean flag to revoke the lease from KMS without trying to remove it from the external provider. Defaults to `False`.
