...
If you're working with C++, the official KMS C++ SDK package is the easiest way to fetch and work with secrets for your application.

## Compatible with C++ 17 and later
The KMS C++ SDK is compatible with C++ 17 capable compilers. This implies GCC 8 or newer, and clang 3.8 or newer. Earlier versions of C++ are unsupported.
...
FetchContent_Declare(
  kms
  GIT_REPOSITORY https://github.com/hanzoai/kms-cpp-sdk.git
...
Below you'll find an example that uses the KMS SDK to fetch a secret with the key `API_KEY` using [Machine Identity Universal Auth](https://kms.hanzo.ai/docs/documentation/platform/identities/universal-auth)

More examples can be found in the [/examples](https://github.com/hanzoai/kms-cpp-sdk/tree/main/examples) folder.

```cpp
#include <iostream>
#include <libkms/KMSClient.h>
...
int main() {
  ...
  try {
    KMS::KMSClient client(
        KMS::ConfigBuilder()
            .withHostUrl("https://kms.hanzo.ai") // Optionally change this to your custom KMS instance URL.
            .withAuthentication(
                KMS::AuthenticationBuilder()
                    .withUniversalAuth("<machine-identity-universal-auth-client-id>", "<machine-identity-universal-auth-client-secret>")
                    .build())
            .build());

    const auto getSecretOptions = KMS::Input::GetSecretOptionsBuilder()
...
## Documentation
The KMS C++ SDK follows a builder pattern for all types of input. Below is a detailed documentation of our currently support methods.

Everything related to the KMS SDK lives inside the `KMS` namespace.

### KMSClient Class
`KMSClient(Config &config)`

```cpp
  KMS::KMSClient client(
      KMS::ConfigBuilder()
          .withHostUrl("https://kms.hanzo.ai")
          .withAuthentication(
              KMS::AuthenticationBuilder()
...
`Config` defines the configuration of the KMS Client itself, such as authentication.

```cpp
KMS::Config config = KMS::ConfigBuilder()
                          .withHostUrl("https://kms.hanzo.ai")
                          .withAuthentication(
                            KMS::AuthenticationBuilder()
                              .withUniversalAuth(clientId, clientSecret)
...
KMS::KMSClient client(config);
```

- `withHostUrl(string)` _(optional)_: Specify a custom KMS host URL, pointing to your KMS instance. Defaults to `https://kms.hanzo.ai`
- `withAuthentication(KMS::Authentication)`: Configure the authentication that will be used by the SDK. See [Authentication Class](#authentication-class) for more details.
- `build()`: Returns the `Config` object with the options you configured.

### Authentication Class
```cpp
KMS::Authentication auth = KMS::AuthenticationBuilder()
                                  .withUniversalAuth(clientId, clientSecret)
                                  .build();
...
#### Create Secret
```cpp
const auto createSecretOptions = KMS::Input::CreateSecretOptionsBuilder()
...
```cpp
  const auto updateSecretOptions = KMS::Input::UpdateSecretOptionsBuilder()
                                    .withEnvironment("<env-slug>")
                                    .withProjectId("<project-id>")
...
```cpp
const auto deleteSecretOptions = KMS::Input::DeleteSecretOptionsBuilder()
                                  .withEnvironment("<env-slug>")
                                  .withProjectId("<project-id>")
                                  .withSecretKey("<secret-key>")
                                  .withType("shared")
...
#### List Secrets
```cpp
const auto listSecretsOptions = KMS::Input::ListSecretOptionsBuilder()
