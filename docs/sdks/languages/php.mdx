...
If you're working with PHP, the official KMS PHP SDK package is the easiest way to fetch and work with secrets for your application.

## Installation

```bash
composer require kms/php-sdk
```
...
$sdk = new KMSSDK('https://kms.hanzo.ai');
...
// List secrets
$params = new \KMS\SDK\Models\ListSecretsParameters(
    environment: "dev",
    secretPath: "/",
...
```php
use KMS\SDK\Models\ListSecretsParameters;
...
```php
use KMS\SDK\Models\CreateSecretParameters;
...
```php
use KMS\SDK\Models\GetSecretParameters;
...
```php
use KMS\SDK\Models\UpdateSecretParameters;
...
```php
use KMS\SDK\Models\DeleteSecretParameters;
