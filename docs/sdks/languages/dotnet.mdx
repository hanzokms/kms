...
If you're working with .NET, the official KMS .NET SDK package is the easiest way to fetch and work with secrets for your application. 
...
public class Program {
  public static void Main(string[] args) {
    ...
    var settings = new KMSSdkSettingsBuilder()
        .WithHostUri("http://localhost:8080") // Optional. Will default to https://kms.hanzo.ai
        .Build();
...
```vb
Imports KMS.Sdk
Imports KMS.Sdk.Model

Module Program
    Sub Main(args As String())
        Dim settings = New KMSSdkSettingsBuilder() _
            .WithHostUri("https://kms.hanzo.ai") _
            .Build()
        
        Dim kmsClient As New KMSClient(settings)
...
3. `Pki()`: Programmatically interact with the KMS PKI.
...
The `Secrets()` sub-class handles operations related to the KMS secrets management product.
