...
Integrate secrets management into your developer portal with the Backstage KMS plugin suite. This plugin provides a seamless interface to manage your [KMS](https://kms.hanzo.ai) secrets directly within Backstage, including full support for environments and folder structure.
...
yarn --cwd packages/app add @hanzo/kms-backstage-plugin-kms
```
...
```bash
# From your Backstage root directory
yarn --cwd packages/backend add @hanzo/kms-backstage-backend-plugin-kms
```
...
backend.add(import('@hanzo/kms-backstage-backend-plugin-kms'));
...
```tsx
import { kmsPlugin } from '@hanzo/kms-backstage-plugin-kms';
...
```tsx
import { EntityKMSContent } from '@hanzo/kms-backstage-plugin-kms';
