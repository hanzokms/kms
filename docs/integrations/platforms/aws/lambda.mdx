...
description: "How to use KMS secrets in AWS Lambda"
---

Learn how to sync KMS secrets to AWS Lambda regardless of how you deploy your function. This guide covers the following strategies:
...
### 1. Fetch secrets at runtime with KMS SDKs

If you control the Lambda code, the simplest method is to fetch secrets directly from KMS using one of our SDKs.  
You can read more about the KMS SDKs [here](/sdks/overview).

### 2. Push via secret sync

Configure a secret sync from your KMS project, and KMS will keep your Secrets Manager or Parameter Store values up to date. Your Lambda function can then reference those secrets directly.  
...
For straightforward workflows or quick rotations, you can push KMS secrets directly into Lambda environment variables using the AWS CLI.
...
Learn more about `hanzo-kms export` [here](/cli/commands/export#kms-export).
...
<Tip>
  Automate this step in CI/CD. Run `hanzo-kms export` using an KMS Token
  scoped to your project and environment, and trigger the sync as part of your
  deployment workflow. Learn more about the [KMS
  Token](/cli/commands/export#kms-export:kms-token).
</Tip>
