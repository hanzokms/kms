...
<Info> 
  Please note that we highly recommend using `hanzo-kms login` for local development.
</Info>
...
You can use the `hanzo-kms login --method=universal-auth` command to directly obtain a universal auth access token and set it as an environment variable.

```bash
  export KMS_TOKEN=$(hanzo-kms login --method=universal-auth --client-id=<your-client-id> --client-secret=<your-client-secret> --silent --plain)
```

<Warning>
  In production scenarios, please to avoid using the `hanzo-kms login` command and instead use a [machine identity](../../documentation/platform/identities/machine-identities).
</Warning>
...
docker run --rm --env-file <(hanzo-kms export --format=dotenv) alpine printenv
```
