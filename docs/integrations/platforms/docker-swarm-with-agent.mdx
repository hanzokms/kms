...
Specifically, we'll set up a sidecar container using the [KMS Agent](/integrations/platforms/kms-agent), which authenticates with KMS to retrieve secrets and access tokens. 
...
- Familiarity with the [KMS Agent](/integrations/platforms/kms-agent)
...
<Steps>
  <Step title="Cloning the Guide Assets Repository">
    Start by cloning the [KMS guide assets repository](https://github.com/hanzoai/kms-guides.git) from Github. This repository includes necessary assets for this and other KMS guides. Focus on the `docker-swarm-with-agent` sub-directory, which we'll use as our working directory.
  </Step>
  ...
  <Step title="Creating the Docker Compose File">
    ...
    services:
      kms-agent:
        ...
        image: ghcr.io/hanzoai/kms-cli:0.18.0
