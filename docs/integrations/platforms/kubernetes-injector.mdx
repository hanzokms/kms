...
The injector is namespace-agnostic, and will watch for pods in any namespace, but will only patch pods that have the `org.kms.hanzo.ai/inject` annotation set to `true`.
...
We're looking to add support for other Windows versions in the future. If you're using a different Windows version, please let us know by opening [an issue](https://github.com/hanzoai/kms-agent-injector/issues/new), and we'll look into adding support for your desired version as soon as possible.
...
<AccordionGroup>
  <Accordion title="Run Agent Injector on Windows Server 2019">
    ...
    ```yaml values.yaml
    image:
      repository: ghcr.io/hanzoai/kms-agent-injector
    ...
    ```bash
    helm install --generate-name kms-helm-charts/kms-agent-injector -f values.yaml
    ```
  </Accordion>
...
<AccordionGroup>
  <Accordion title="org.kms.hanzo.ai/inject">
  ...
  <Accordion title="org.kms.hanzo.ai/inject-mode">
    ...
    - `sidecar`: The sidecar method will create a sidecar container for the pod that will render the secrets into a shared volume mount within the pod. The agent sidecar container will run alongside the main container in the pod. This means that the secrets rendered will always be in sync with your KMS secrets.
  ...
  <Accordion title="org.kms.hanzo.ai/agent-config-map">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-cache-enabled">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-image">
    Specify a custom agent image to use for the agent sidecar / init container(s). Example: `ghcr.io/hanzoai/kms-cli:0.43.32`.
    If not specified, the most recent stable version of the KMS Agent will be used.
  </Accordion>

  <Accordion title="org.kms.hanzo.ai/agent-revoke-on-shutdown">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-client-max-retries">
    How many times to retry failed API requests such as authentication, secret retrieval, etc. Defaults to `3` retries. Refer to the [Retrying mechanism](/integrations/platforms/kms-agent#retrying-mechanism) documentation for more information on how to configure the retry strategy.
  </Accordion>

  <Accordion title="org.kms.hanzo.ai/agent-client-max-delay">
    The maximum delay between retries. Defaults to `5s` (5 seconds). Refer to the [Retrying mechanism](/integrations/platforms/kms-agent#retrying-mechanism) documentation for more information on how to configure the retry strategy.
  </Accordion>

  <Accordion title="org.kms.hanzo.ai/agent-client-base-delay">
    The base delay between retries. Defaults to `200ms` (200 milliseconds). Refer to the [Retrying mechanism](/integrations/platforms/kms-agent#retrying-mechanism) documentation for more information on how to configure the retry strategy.
  </Accordion>

  <Accordion title="org.kms.hanzo.ai/agent-limits-cpu">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-requests-cpu">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-limits-memory">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-requests-memory">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-limits-ephemeral">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-requests-ephemeral">
...
<Accordion title="org.kms.hanzo.ai/agent-set-security-context">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-security-context-run-as-user">
    ...
    <Note>
      This annotation is only applicable when `org.kms.hanzo.ai/agent-set-security-context` is set to `true`.
    </Note>
  </Accordion>

  <Accordion title="org.kms.hanzo.ai/agent-security-context-run-as-group">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-security-context-read-only-root-filesystem">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-security-context-privileged">
  ...
  <Accordion title="org.kms.hanzo.ai/agent-security-context-allow-privilege-escalation">
...
<Accordion title="kms.address">
  The address of your KMS instance. This field is optional and will default to `https://kms.hanzo.ai` if not provided.
</Accordion>

<Accordion title="kms.revoke-credentials-on-shutdown">
  ...
  **Note:** In disaster events such as cluster power outages, a `SIGTERM` signal won't be sent to the agents container, and the credentials will not be revoked.
  ...
  <Note>
    ...
    It's recommended to use the annotation `org.kms.hanzo.ai/agent-revoke-on-shutdown: "true"` instead of configuring the revoke on shutdown on the config map. Refer to the [Supported annotations](/integrations/platforms/kubernetes-injector#supported-annotations) documentation for more information on how to configure the revoke on shutdown through annotations.
  </Note>
</Accordion>
...
<Accordion title="kms.auth.config.identity-id">
  The ID of the machine identity to use for Kubernetes or LDAP authentication. This field is required if the `kms.auth.type` is set to `kubernetes`.
</Accordion>

<Accordion title="kms.auth.config.username">
  The LDAP username to use for LDAP authentication.
  This field is required if the `kms.auth.type` is set to `ldap-auth`.
</Accordion>

<Accordion title="kms.auth.config.password">
  The LDAP password to use for LDAP authentication.
  This field is required if the `kms.auth.type` is set to `ldap-auth`.
</Accordion>

<Accordion title="kms.retry-strategy.max-retries">
  How many times to retry failed API requests such as authentication, secret retrieval, etc. Defaults to `3` retries. Refer to the [Retrying mechanism](/integrations/platforms/kms-agent#retrying-mechanism) documentation for more information on how to configure the retry strategy.
...
<Accordion title="kms.retry-strategy.max-delay">
  The maximum delay between retries. Defaults to `5s` (5 seconds). Refer to the [Retrying mechanism](/integrations/platforms/kms-agent#retrying-mechanism) documentation for more information on how to configure the retry strategy.
...
<Accordion title="kms.retry-strategy.base-delay">
  The base delay between retries. Defaults to `200ms` (200 milliseconds). Refer to the [Retrying mechanism](/integrations/platforms/kms-agent#retrying-mechanism) documentation for more information on how to configure the retry strategy.
...
### Authentication
The KMS Agent Injector supports Machine Identity [Kubernetes Auth](/documentation/platform/identities/kubernetes-auth) and [LDAP Auth](/documentation/platform/identities/ldap-auth) authentication.
...
To use the config map in your pod, you will need to add the `org.kms.hanzo.ai/agent-config-map` annotation to your pod's deployment. The value of the annotation is the name of the config map you created above. The config map must be in the same namespace as the pod you're injecting into.
```yaml
apiVersion: v1
kind: Pod
metadata:
  ...
  labels:
    app: demo
  annotations:
    org.kms.hanzo.ai/inject: "true" # Set to true for the injector to patch the pod on create/update events
    org.kms.hanzo.ai/inject-mode: "init" # The mode to use to inject the secrets into the pod. init|sidecar
    org.kms.hanzo.ai/agent-config-map: "name-of-config-map" # The name of the config map that you created above, which contains all the settings for injecting the secrets into the pod
spec:
  # ...
```
...
To inject secrets into your pod, you will need to add the `org.kms.hanzo.ai/inject: "true"` annotation to your pod's deployment.

The `org.kms.hanzo.ai/agent-config-map` annotation will point to the config map we created in the previous step. It's important that the config map is in the same namespace as the pod.
...
metadata:
  ...
  labels:
    app: nginx
  annotations:
    org.kms.hanzo.ai/inject: "true"
    org.kms.hanzo.ai/inject-mode: "init"
    org.kms.hanzo.ai/agent-config-map: "nginx-kms-config-map"
spec:
  containers:
...
<Accordion title="The pod is stuck in `Init` state">
  ...
  You can check the logs of the hanzo-kms init container by running:
  ```bash
  # For deployments
