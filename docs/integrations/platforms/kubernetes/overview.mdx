...
<Tabs>
  ...
  <Tab title="Supported Kubernetes distributions">
    The KMS Operator has been tested successfully in the following hosted Kubernetes environments:
    ...
    It may work in other Kubernetes distributions, but those are not officially supported. Please report any issues [here](https://github.com/hanzoai/kms/issues).
  </Tab>
</Tabs>
...
**Controllers:** `KmsSecret`, `KmsPushSecret`, `KmsDynamicSecret`
...
To configure global settings that will apply to all CRD instances (`KmsSecret`, `KmsPushSecret`, and `KmsDynamicSecret`), you can define these configurations in a Kubernetes ConfigMap.
...
| -------- | --------------------------------------------------------------------------------- | ----------------------------- |
| hostAPI  | If `hostAPI` in a CRD instance is left empty, this value will be used | https://kms.hanzo.ai/api |
