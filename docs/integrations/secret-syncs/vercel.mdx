...
**Prerequisites:**
    ...
    - Set up and add secrets to [KMS Cloud](https://kms.hanzo.ai)
...
<Tabs>
    <Tab title="KMS UI">
        ...
        ![Configure Options](/images/secret-syncs/vercel/vercel-options.png)
            ...
            <Note>
                Vercel does not expose the values of [sensitive environment variables](https://vercel.com/docs/environment-variables/sensitive-environment-variables), so KMS cannot import them during the initial sync. As a result, these secrets are created in KMS with empty values. After the first sync, you'll need to manually re-enter their values in KMS to ensure both platforms stay aligned.
            </Note>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl    --request POST \
        --url https://kms.hanzo.ai/api/v1/secret-syncs/vercel \
