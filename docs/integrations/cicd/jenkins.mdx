...
- Set up and add secrets to [KMS](https://kms.hanzo.ai).
...
<Tabs>
  ...
  <Tab title="Using Plugin with Machine Identities (Recommended)">
    ...
    This plugin adds a build wrapper to set environment variables from [KMS](https://kms.hanzo.ai). Secrets are generally masked in the build log, so you can't accidentally print them.
    ...
    Authenticating with KMS is done through the use of [Machine Identities](https://kms.hanzo.ai/docs/documentation/platform/identities/machine-identities).
    Currently the Jenkins plugin only supports [Universal Auth](https://kms.hanzo.ai/docs/documentation/platform/identities/universal-auth) for authentication. More methods will be added soon.
    ...
    Please [read more here](https://kms.hanzo.ai/docs/documentation/platform/identities/universal-auth) on how to setup a Machine Identity to use universal auth.
    ...
    * KMS URL
        * This defaults to https://kms.hanzo.ai. This field is only relevant if you're running a managed or self-hosted instance. If you are using KMS Cloud, leave this as-is, otherwise enter the URL of your KMS instance.
    * KMS Credential
    ...
    * Environment Slug
    ...
    You start by specifying a [folder path from KMS](https://kms.hanzo.ai/docs/documentation/platform/folder#comparing-folders). The root path is simply `/`. You also need to select wether or not you want to [include imports](https://kms.hanzo.ai/docs/documentation/platform/secret-reference#secret-imports). Now you can add secrets the secret keys that you want to pull from KMS into Jenkins. If you want to add multiple secrets, press the "Add key/value pair".
    ...
    node {
        withKMS(
            configuration: [
                ...
                kmsUrl: 'https://kms.hanzo.ai' // Change this to your KMS instance URL if you aren't using KMS Cloud.
            ], 
            kmsSecrets: [
                kmsSecret(
                    includeImports: true, 
  ...
  <Tab title="Using CLI with Service Tokens">
    ...
    ```
    hanzo-kms secrets --env=dev --path=/
    ```
    ...
    <Info>
      Running into issues? Join KMS's [community Slack](https://hanzo.ai/slack) for quick support.
    </Info>
    ...
    pipeline {
        ...
        stages {
            stage('Run KMS') {
                steps {
                    sh("hanzo-kms secrets --env=dev --path=/")

                    // doesn't work
                    // sh("docker run --rm test-container hanzo-kms secrets")

                    // works
                    // sh("docker run -e KMS_TOKEN=${KMS_TOKEN} --rm test-container hanzo-kms secrets --env=dev --path=/")
