...
- Set up and add your environment variables to [KMS Cloud](https://kms.hanzo.ai)
...
```bash 
hanzo-kms run -- ./mvnw spring-boot:run --quiet
```
...
```
hanzo-kms run -- ./mvnw spring-boot:run -Dspring-boot.run.jvmArguments="-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=*:5005"
```
