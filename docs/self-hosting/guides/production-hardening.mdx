...
docker run --cap-drop=ALL ghcr.io/hanzoai/kms:latest
```
...
docker run ghcr.io/hanzoai/kms:v0.93.1-postgres
```
...
docker run --memory=1g --cpus=0.5 ghcr.io/hanzoai/kms:latest
```
...
**Regular updates**. Monitor [Docker Hub](https://hub.docker.com/r/ghcr.io/hanzoai/kms/tags) for new releases and update your image tags regularly.
...
spec:
  template:
    spec:
      ...
      containers:
        - name: kms
          image: ghcr.io/hanzoai/kms:v0.93.1-postgres
          securityContext:
...
**Regular updates**. Monitor [Docker Hub](https://hub.docker.com/r/ghcr.io/hanzoai/kms/tags) for new releases and update your deployment manifests with new image tags regularly.
...
ExecStart=/opt/ghcr.io/hanzoai/kms-linux-amd64
Restart=always
RestartSec=10
...
sudo chmod 755 /opt/ghcr.io/hanzoai/kms-linux-amd64
sudo chown kms:kms /opt/ghcr.io/hanzoai/kms-linux-amd64
...
**Regular security updates**. Monitor the [KMS repository](https://github.com/hanzoai/kms) for security updates and apply them promptly.
...
KMS is compliant with FIPS 140-3, meeting U.S. and Canadian government cryptographic standards through validated cryptographic modules.
This certification is designed for organizations that require government-approved encryption implementations.
To deploy a FIPS-compliant instance, use the [ghcr.io/hanzoai/kms-fips](https://hub.docker.com/r/ghcr.io/hanzoai/kms-fips) Docker image, available to Enterprise customers.
Our FIPS 140-3 attestation letter is available in the [KMS Trust Center](https://trust.kms.hanzo.ai/).

#### SOC 2 Compliance

KMS is SOC 2 compliant, demonstrating adherence to rigorous security, availability, and confidentiality standards established by the American Institute of CPAs (AICPA).
This certification validates our security controls and operational practices for organizations requiring third-party audited security assurance. Our SOC 2 report is available in the [KMS Trust Center](https://trust.kms.hanzo.ai/).

#### HIPAA Compliance

KMS is HIPAA compliant, meeting the security and privacy requirements of the Health Insurance Portability and Accountability Act.
...
Our HIPAA certification is available in the [KMS Trust Center](https://trust.kms.hanzo.ai/).