...
title: "Replication"
description: "Learn how KMS supports multi-region replication"

---

<Info>
    KMS replication is a paid feature.

    If you're using KMS Cloud, then it is available under the **Enterprise Tier**. If you're self-hosting KMS,
    then you should contact team@kms.hanzo.ai to purchase an enterprise license to use it.
</Info>

Multi-region replication is available in KMS Enterprise to support globally distributed deployments. Understanding the architecture, use cases, and operational considerations is essential before implementing this feature in production environments.

KMS uses a primary/secondary (1:N) architecture with asynchronous PostgreSQL replication. This design prioritizes high availability and minimal read latency for applications deployed across multiple geographic regions.
...
- **Zero Client Changes**: Existing KMS integrations, SDKs, and CLI tools work without modification. Regional instances route write operations to the primary while handling reads locally. Authentication tokens and API keys function identically across all instances.
...
KMS distinguishes between _primary_ and _secondary_ instances. The primary holds write authority and is the sole instance permitted to modify the PostgreSQL database. Secondary instances handle read traffic locally and proxy write operations to the primary.

## Infrastructure components

Two data stores form KMS's persistence layer:
...
The KMS application servers are stateless and therefore hold no persistent data internally. This design simplifies regional deployment and horizontal scaling.
...
<Tabs>
  ...
  <Tab title="Secondary region configuration">
   ...
   1. `KMS_PRIMARY_INSTANCE_URL`: The primary region's KMS API endpoint
...
Understanding the implementation details can help evaluate whether KMS's replication characteristics align with your requirements. 
...
KMS relies on PostgreSQL's native replication, which provides:
...
- **Promotion capability**: Read replicas can be promoted to primary during disaster recovery. Promotion requires updating KMS configuration to designate the promoted instance as primary and reconfiguring other secondaries.
...
All KMS instances must run identical versions (mixing versions risks database schema mismatches or incompatible API behavior). Database migrations execute only on the primary and replicate to secondaries through standard PostgreSQL mechanisms.
...
KMS uses versioned caching rather than active invalidation:
