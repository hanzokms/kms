...
KMS includes a built-in workaround that detects version mismatches and triggers a page reload. While functional, this introduces a noticeable delay for users during deployments.
...
At KMS, we use **CloudFront + S3** for this purpose, but you can use any CDN and object storage combination that fits your infrastructure.

## Exporting Assets

KMS provides a built-in command to export frontend assets from the Docker image:

```bash
# Export as tar archive to stdout
docker run --rm ghcr.io/hanzoai/kms npm run --silent assets:export > assets.tar
...
docker run --rm -v $(pwd)/cdn-assets:/output \
  ghcr.io/hanzoai/kms npm run --silent assets:export /output
```
...
1. **Build** your new Docker image (or pull the official KMS image)
...
docker run --rm ghcr.io/hanzoai/kms:$VERSION npm run --silent assets:export > assets.tar
tar -xf assets.tar
