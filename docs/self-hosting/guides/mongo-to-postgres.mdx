...
<Steps>
  <Step title="Clone KMS Repository">
    ...
    ```
    git clone -b kms/v0.46.11-postgres https://github.com/hanzoai/kms.git
    ```
  </Step>
...
KMS's [Docker Hub repository](https://hub.docker.com/r/ghcr.io/hanzoai/kms) uses different tagging conventions to indicate which database backend is used:

- **Before v0.46.11**
  - KMS ran on MongoDB backend

- **After v0.46.11** 
  - Version tags started to be suffixed with `-postgres`
  - KMS transitioned to PostgreSQL backend

- **After v0.147.0**
  - KMS remains on PostgreSQL backend
