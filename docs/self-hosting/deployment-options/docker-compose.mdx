...
description: "Deploy KMS using Docker Compose for development, testing, or small-scale production environments."
---

Learn how to deploy **KMS** using Docker Compose. This method runs KMS and its dependencies (PostgreSQL and Redis) as containers on a single Docker host. It's ideal for trying out KMS, development environments, or small-scale deployments that don't require high availability.
...
<Warning>
  ...
  To run KMS in a highly available manner, see the [Docker Swarm guide](/self-hosting/deployment-options/docker-swarm) or [Kubernetes guide](/self-hosting/deployment-options/kubernetes-helm).
</Warning>

## System Requirements

The following are minimum requirements for running KMS with Docker Compose:
...
These requirements include resources for KMS, PostgreSQL, and Redis containers. For larger deployments with many secrets or users, increase resources accordingly.
...
<Steps>
  ...
  <Step title="Download the Docker Compose file">
    You can obtain the KMS docker compose file by using a command-line downloader such as `wget` or `curl`. If your system doesn't have either of these, you can use an equivalent command that works with your machine.
  ...
  <Step title="Configure environment variables">
    KMS requires a set of credentials used for connecting to dependent services such as Postgres, Redis, etc. The default credentials can be downloaded using one of the commands listed below.
  ...
  <Step title="Start KMS">
    ...
    - **backend**: The main KMS application (exposed on host port 80, internal port 8080)
  ...
  <Step title="Verify the deployment">
    ...
    You should see all three services with a status of `Up`. Next, verify KMS is responding:
    ...
    <Tip>
      The first user to sign up becomes the instance administrator. Make sure to complete this step before exposing KMS to others.
    </Tip>
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Firewall Configuration">
    Configure your firewall to allow traffic to KMS while securing other services:

    **Required ports:**
    | Port | Service | Access |
    |------|---------|--------|
    | 80 | HTTP (KMS) | Public (or internal network) |
  ...
  <Accordion title="SMTP/Email Configuration">
    KMS uses email for user invitations, password resets, and notifications. Configure SMTP by adding these variables to your `.env` file:
    ...
    **Common SMTP providers:**
    ...
    After updating the `.env` file, restart KMS:
  ...
  <Accordion title="Enable HTTPS with NGINX">
    We recommend securing your KMS deployment with HTTPS using a reverse proxy like NGINX.
  ...
  <Accordion title="Upgrade Instructions">
    Keeping KMS up-to-date ensures you receive the latest features and security patches.
    ...
    **4. Verify the upgrade:**
    ...
    Watch for successful database migration messages. See the [Upgrade Guide](/self-hosting/guides/upgrading-kms) for more details.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Database connection errors">
    ...
    **Check KMS logs for connection errors:**
    ```bash
  ...
  <Accordion title="Cannot access KMS in browser">
    ...
    **Verify SITE_URL is correct** in your `.env` file and matches how you're accessing KMS.
  </Accordion>
  ...
  <Accordion title="Email/SMTP not working">
    ...
    **Check KMS logs for email errors:**
    ```bash
  ...
  <Accordion title="Reset admin password">
    ...
    For security reasons, passwords are hashed and cannot be directly reset in the database. Use the "Forgot Password" flow if SMTP is configured, or contact KMS support for assistance.
  </Accordion>
</AccordionGroup>

---

Your KMS instance should now be running on port `80` (or `443` if using TLS). Visit `http://localhost:80` or `https://<your-domain>` to access your instance.
