...
<Steps>
  <Step title="Pull the KMS Docker image">
    Visit [Docker Hub](https://hub.docker.com/r/ghcr.io/hanzoai/kms/tags) and select a version of KMS image you would like to deploy.
    Then run the following command in your terminal to pull the specific KMS Docker image.
    
    ```
    docker pull ghcr.io/hanzoai/kms:<version>
    ```
