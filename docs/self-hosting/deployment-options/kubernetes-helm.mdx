...
description: "Learn how to deploy KMS on Kubernetes using our official Helm chart."
---

Learn how to deploy **KMS** on Kubernetes using the official Helm chart. This method is ideal for production environments that require scalability, high availability, and integration with existing Kubernetes infrastructure.
...
The following are minimum requirements for running KMS on Kubernetes:
...
<Steps>
  <Step title="Create a namespace">
    Create a dedicated namespace for KMS to isolate resources:

    ```bash
    kubectl create namespace kms
    ```

    All subsequent commands will use this namespace. You can also add `-n kms` to each kubectl command if you prefer not to set a default context.
  </Step>

  <Step title="Add the Helm repository">
    Add the KMS Helm charts repository and update your local cache:
  ...
  <Step title="Create the secrets">
    KMS requires a Kubernetes secret named `kms-secrets` containing essential configuration. Create this secret in the same namespace where you'll deploy the chart.
  ...
  <Step title="Create values.yaml">
    ...
    kms:
      image:
        ...
        tag: "v0.151.0"  # Check https://hub.docker.com/r/ghcr.io/hanzoai/kms/tags for latest
    ...
    For all available configuration options, see the [full values.yaml reference](https://raw.githubusercontent.com/KMS/kms/main/helm-charts/kms-standalone/values.yaml).
  </Step>

  <Step title="Install the Helm chart">
    Deploy KMS using Helm:
    ...
    This command installs KMS if it doesn't exist, or upgrades it if it does.
  </Step>
  ...
  <Step title="Verify the deployment">
    ...
    <Tip>
      The first user to sign up becomes the instance administrator. Complete this step before exposing KMS to others.
    </Tip>
  </Step>
</Steps>
...
To view logs from KMS pods:

```bash
# View logs from all KMS pods
...
kubectl logs -n kms <pod-name> --previous
```
...
kubectl logs -n kms -l app.kubernetes.io/name=redis -f
```

### Scaling the Deployment

KMS's application layer is stateless, so you can scale horizontally:

```bash
# Scale to 4 replicas
kubectl scale deployment -n kms kms --replicas=4
```
...
1. **Back up your database** before upgrading:
   ```bash
   kubectl exec -n kms postgresql-0 -- pg_dump -U kms kmsDB > backup_$(date +%Y%m%d).sql
   ```
...
To completely remove KMS from your cluster:
...
<AccordionGroup>
  <Accordion title="SMTP/Email Configuration">
    KMS uses email for user invitations, password resets, and notifications. Add SMTP configuration to your secrets:
    ...
    **Common SMTP providers:**
    ...
    After updating secrets, restart the KMS pods:

    ```bash
    kubectl rollout restart deployment/kms -n kms
    ```
  </Accordion>
  ...
  <Accordion title="Prometheus Monitoring">
    KMS exposes Prometheus metrics when enabled.
...
<AccordionGroup>
  <Accordion title="Pods stuck in Pending state">
    **Check pod events:**
    ```bash
    kubectl describe pod -n kms <pod-name>
    ```

    **Common causes:**
    - Insufficient cluster resources: Check node capacity with `kubectl describe nodes`
    - PVC not bound: Check PVC status with `kubectl get pvc -n kms`
  ...
  <Accordion title="Pods in CrashLoopBackOff">
    ...
    **Common causes:**
    - Missing or invalid secrets: Verify `kms-secrets` exists and contains required keys
    ...
    **Verify secrets:**
    ```bash
    kubectl get secret kms-secrets -n kms -o yaml
    ```
  </Accordion>

  <Accordion title="Cannot access KMS via Ingress">
    **Check ingress status:**
    ```bash
    kubectl get ingress -n kms
    kubectl describe ingress -n kms kms
    ```
  ...
  <Accordion title="Database connection errors">
    **Check PostgreSQL pod:**
    ...
    kubectl logs -n kms postgresql-0
    ```

    **Test database connectivity:**
    ```bash
    kubectl exec -it -n kms postgresql-0 -- psql -U kms -d kmsDB -c "SELECT 1"
    ```

    **For external databases:**
    - Verify the connection string in `kms-secrets`
  ...
  <Accordion title="Redis connection errors">
    **Check Redis pod:**
    ...
    kubectl logs -n kms redis-master-0
    ```

    **Test Redis connectivity:**
    ```bash
    kubectl exec -it -n kms redis-master-0 -- redis-cli ping
    ```

    **For external Redis:**
    - Verify the `REDIS_URL` in `kms-secrets`
  ...
  <Accordion title="Performance issues">
    ...
    **Check for resource throttling:**
    ```bash
    kubectl describe pod -n kms <pod-name> | grep -A5 "Limits\|Requests"
    ```
...
<Accordion title="Complete values.yaml example">
      ...
      kms:
        # -- Enable KMS chart deployment
        enabled: true
        # -- Sets the name of the deployment within this chart
        name: kms

        autoBootstrap:
          # -- Enable auto-bootstrap of the KMS instance
          enabled: false

          image:
            # -- KMS KMS CLI image tag version
            tag: "0.41.86"
          ...
          # -- KMS organization to create in the KMS instance during auto-bootstrap
        ...
        # -- Override for the full name of KMS resources in this deployment
        fullnameOverride: ""
        # -- Custom annotations for KMS pods
        podAnnotations: {}
        # -- Custom annotations for KMS deployment
        deploymentAnnotations: {}
        # -- Number of pod replicas for high availability
        replicaCount: 2

        image:
          # -- Image repository for the KMS service
          repository: ghcr.io/hanzoai/kms
          # -- Specific version tag of the KMS image. View the latest version here https://hub.docker.com/r/ghcr.io/hanzoai/kms
          tag: "v0.151.0"
        ...
        # -- Kubernetes Secret reference containing KMS root credentials
        kubeSecretRef: "kms-secrets"

        service:
          # -- Custom annotations for KMS service
          annotations: {}
        ...
        resources:
          limits:
            # -- Memory limit for KMS container
            memory: 1000Mi
          requests:
            # -- CPU request for KMS container
            cpu: 350m
...
Your KMS instance should now be running on Kubernetes. Access it via the ingress hostname you configured, or use `kubectl port-forward` for local testing.
